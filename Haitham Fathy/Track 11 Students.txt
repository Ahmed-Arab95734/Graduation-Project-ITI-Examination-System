/*
============================================================================
== SCRIPT TO POPULATE DATA FOR TRACK 11: FULL STACK WEB DEVELOPMENT USING .NET
==
== STARTING IDs: 20000
== STUDENTS PER IBT: 25
...
-- --- ID Counters ---
DECLARE @StudentID_Start INT = 20000;
DECLARE @JobID_Start INT = 20000;
DECLARE @CertID_Start INT = 20000;
...
INSERT INTO @CertProviders (Name) VALUES (N'Coursera'), (N'Udemy'), (N'Udacity'), (N'edX'), (N'Mahara Tech'), (N'Microsoft Learn'), (N'DataCamp'), (N'Pluralsight');
...
-- --- Track 11 (.NET) Specific Data ---

DECLARE @NET_Positions TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(100));
INSERT INTO @NET_Positions (Name) VALUES
(N'.NET Developer'), (N'Full Stack Engineer (C#)'), (N'Backend Developer (.NET)'), (N'Azure Cloud Engineer'),
(N'Junior Software Engineer'), (N'Web Developer'), (N'API Specialist (ASP.NET)'), (N'Angular Developer'),
(N'Software Engineer - .NET');

DECLARE @NET_Companies TABLE (Company_ID INT);
INSERT INTO @NET_Companies (Company_ID) VALUES 
(1),  -- Microsoft
...
DECLARE @NET_Certs TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(200));
INSERT INTO @NET_Certs (Name) VALUES
(N'Microsoft Certified: Azure Fundamentals (AZ-900)'), (N'Microsoft Certified: Developing Solutions for Azure (AZ-204)'),
(N'Angular - The Complete Guide (Udemy)'), (N'C# Basics for Beginners (Mahara Tech)'),
(N'Entity Framework Core in Depth'), (N'Microservices with .NET and Docker'),
(N'.NET API Development (Udacity)'), (N'Blazor Fundamentals');

DECLARE @NET_Jobs TABLE (ID INT IDENTITY(1,1), Descr NVARCHAR(1000));
INSERT INTO @NET_Jobs (Descr) VALUES
(N'Build a small REST API for a store inventory system.'), (N'Fix bugs in an existing ASP.NET MVC application.'),
(N'Create a simple CRUD (Create, Read, Update, Delete) app with EF Core.'), (N'Develop a new component in Angular for a client dashboard.'),
(N'Deploy a small web app to Azure App Service.'), (N'Write unit tests for a C# class library using xUnit.'),
(N'Refactor legacy C# code to use async/await patterns.'), (N'Design a simple database schema and implement it with EF Core Migrations.'),
(N'Integrate a third-party payment gateway API into a .NET web app.'), (N'Build a landing page with ASP.NET Core and Bootstrap.'),
(N'Optimize a slow-running SQL query used by an application.'), (N'Set up authentication and authorization using ASP.NET Identity.'),
(N'Create a basic console application for data processing.'), (N'Develop a small microservice using .NET.'),
(N'Fix styling issues on a website using CSS and Angular.'), (N'Build a simple app with Blazor WebAssembly.'),
(N'Consult on migrating a .NET Framework app to .NET Core.');
...
WHERE ibt.Track_ID = 11; -- .NET Full Stack

-- --- Target Courses (Track 11) ---
...
PRINT 'Starting data generation for Track 11: .NET Full Stack...';
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT TOP 1 @CertName = Name FROM @NET_Certs ORDER BY NEWID();
        ...
        BEGIN
            ...
            SELECT TOP 1 @JobDesc = Descr FROM @NET_Jobs ORDER BY NEWID();
        ...
        BEGIN
            ...
            BEGIN
                ...
                -- Get random company from the .NET list
                SELECT TOP 1 @Company_ID = Company_ID FROM @NET_Companies ORDER BY NEWID();
                
                -- Get random position
                SELECT TOP 1 @Position = Name FROM @NET_Positions ORDER BY NEWID();
