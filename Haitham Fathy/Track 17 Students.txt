/*
============================================================================
== SCRIPT TO POPULATE DATA FOR TRACK 17: 3D MODELING
==
== STARTING IDs: 32000
== STUDENTS PER IBT: 25
...
-- --- ID Counters ---
DECLARE @StudentID_Start INT = 32000;
DECLARE @JobID_Start INT = 32000;
DECLARE @CertID_Start INT = 32000;
...
INSERT INTO @CertProviders (Name) VALUES (N'Coursera'), (N'Udemy'), (N'Udacity'), (N'Autodesk'), (N'Maxon (ZBrush)'), (N'Skillshare'), (N'Pluralsight'), (N'Gnomon');

DECLARE @FreelanceSites TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(255));
INSERT INTO @FreelanceSites (Name) VALUES (N'Upwork'), (N'Freelancer'), (N'LinkedIn'), (N'ArtStation'), (N'CGTrader'), (N'Fiverr'), (N'Mostaql'); -- ArtStation/CGTrader more relevant


-- --- Track 17 (3D Modeling) Specific Data ---

DECLARE @M3D_Positions TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(100));
INSERT INTO @M3D_Positions (Name) VALUES
(N'3D Modeler'), (N'Junior 3D Artist'), (N'Character Artist'), (N'Environment Artist'),
(N'Texture Artist'), (N'Hard Surface Modeler'), (N'Architectural Visualizer'), (N'3D Generalist'),
(N'Game Asset Artist');

DECLARE @M3D_Companies TABLE (Company_ID INT);
-- NOTE: Company list doesn't have obvious game studios/animation houses. Using agencies & potentially relevant large corps.
INSERT INTO @M3D_Companies (Company_ID) VALUES 
(16), -- Robusta (Digital Agency - might do 3D for campaigns/AR)
(41), -- ArabyAds (Advertising - needs 3D assets occasionally)
(9),  -- Orascom Construction (Architectural Visualization)
(25), -- Hassan Allam Holding (Architectural Visualization)
(26), -- Arab Contractors (Architectural Visualization)
(40), -- Majid Al Futtaim (Retail/Real Estate - ArchViz/Marketing)
(7),  -- Valeo (Product Visualization / Engineering Models)
(15); -- NAGWA (Educational 3D Models)

DECLARE @M3D_Certs TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(200));
INSERT INTO @M3D_Certs (Name) VALUES
(N'Autodesk Certified Professional: Maya'), 
(N'Autodesk Certified User: 3ds Max'),
(N'ZBrush Certification (if offered officially, else project-based)'), 
(N'Complete Blender Creator (Udemy)'),
(N'Maya for Beginners (Udemy/Pluralsight)'), 
(N'Character Modeling for Games (Gnomon/Udemy)'),
(N'Substance Painter/Designer Fundamentals'), 
(N'Hard Surface Modeling Techniques (Udemy/Skillshare)');

DECLARE @M3D_Jobs TABLE (ID INT IDENTITY(1,1), Descr NVARCHAR(1000));
INSERT INTO @M3D_Jobs (Descr) VALUES
(N'Model a low-poly stylized character based on concept art.'), 
(N'Create a realistic 3D model of a piece of furniture.'),
(N'UV unwrap and texture a game weapon model.'), 
(N'Sculpt a creature head in ZBrush.'),
(N'Model a simple architectural exterior scene in Maya/3ds Max.'), 
(N'Create PBR textures for a sci-fi prop using Substance Painter.'),
(N'Retopologize a high-poly sculpt for game engine use.'), 
(N'Model and texture a set of small environment props (barrels, crates).'),
(N'Optimize an existing high-poly 3D model for real-time rendering.'), 
(N'Create a basic rig for a simple character model.'),
(N'Set up lighting and render a product shot.'), 
(N'Model a vehicle based on blueprints.'),
(N'Sculpt realistic cloth details on a character model.'), 
(N'Texture paint a stylized character model.'),
(N'Create modular environment assets for a game level.'), 
(N'Model a detailed hard-surface object like a robot arm.'),
(N'Prepare a 3D model for 3D printing.');
...
WHERE ibt.Track_ID = 17; -- 3D Modeling

-- --- Target Courses (Track 17) ---
...
PRINT 'Starting data generation for Track 17: 3D Modeling...';
...
BEGIN
    ...
    BEGIN
        ...
        -- Faculty (Weighted - User requested CS/Eng priority, but Fine/Applied Arts also relevant)
        -- Increasing chance slightly for Arts faculties for this track
        ...
        -- LinkedIn standard, Instagram highly relevant for 3D portfolios
        ...
        -- ArtStation/Behance would be better than GitHub here, but sticking to constraints
        IF RAND() > 0.7 -- Less chance for GitHub
             INSERT INTO Student_Social (Student_ID, Social_Type, Social_Url)
        ...
        BEGIN
            ...
            SELECT TOP 1 @CertName = Name FROM @M3D_Certs ORDER BY NEWID();
            SELECT TOP 1 @CertProvider = Name FROM @CertProviders ORDER BY NEWID();
            -- Autodesk/Maxon certs can be expensive
            SET @CertCost = CASE 
                                WHEN @CertProvider IN ('Autodesk', 'Maxon (ZBrush)') THEN (ABS(CHECKSUM(NEWID())) % 150 + 150)*10 -- 1500-3000 range
                                ELSE (ABS(CHECKSUM(NEWID())) % 200) * 10 -- 0 - 1990 range
                            END;
        ...
        DECLARE @JobCount INT = ABS(CHECKSUM(NEWID())) % 6; -- 0 to 5 jobs (3D gigs vary)
        DECLARE @j INT = 0;
        WHILE @j < @JobCount
        BEGIN
            ...
            SELECT TOP 1 @JobDesc = Descr FROM @M3D_Jobs ORDER BY NEWID();
            SET @JobEarn = (ABS(CHECKSUM(NEWID())) % 1500) + 100; -- 100 to 1600 (3D work can pay more)
        ...
        BEGIN
            ...
            BEGIN
                ...
                -- Get random company from the M3D list
                SELECT TOP 1 @Company_ID = Company_ID FROM @M3D_Companies ORDER BY NEWID();
                
                -- Get random position
                SELECT TOP 1 @Position = Name FROM @M3D_Positions ORDER BY NEWID();

                -- Get contract type & salary (Full-Time > Part-Time)
                IF RAND() > 0.30
                BEGIN
                    SET @Contract_Type = N'Full-Time';
                    SET @Salary = (ABS(CHECKSUM(NEWID())) % 9000) + 7000; -- 7k to 16k (Starting 3D roles)
                END
                ELSE
                BEGIN
...
PRINT 'Data generation complete for Track 17.';
