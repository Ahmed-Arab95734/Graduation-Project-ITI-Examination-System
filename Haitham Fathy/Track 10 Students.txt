/*
============================================================================
== SCRIPT TO POPULATE DATA FOR TRACK 10: RENEWABLE ENERGY (Corrected)
==
== STARTING IDs: 18000
== STUDENTS PER IBT: 25
...
-- --- ID Counters ---
DECLARE @StudentID_Start INT = 18000;
DECLARE @JobID_Start INT = 18000;
DECLARE @CertID_Start INT = 18000;
...
(N'Ahmed', N'Male'), (N'Mohamed', N'Male'), (N'Mahmoud', N'Male'), (N'Mostafa', N'Male'), (N'Youssef', N'Male'), (N'Omar', N'Male'), (N'Ali', N'Male'), (N'Khaled', N'Male'), (N'Ibrahim', N'Male'), (N'Tarek', N'Male'), (N'Yassin', N'Male'), (N'Karim', N'Male'),
(N'Fatma', N'Female'), (N'Mariam', N'Female'), (N'Hana', N'Female'), (N'Aya', N'Female'), (N'Salma', N'Female'), (N'Nour', N'Female'), (N'Menna', N'Female'), (N'Sara', N'Female'), (N'Hager', N'Female'), (N'Jana', N'Female'), (N'Farida', N'Female'), (N'Rowan', N'Female');

-- Last Names
DECLARE @LastNames TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(50));
INSERT INTO @LastNames (Name) VALUES
(N'El-Masry'), (N'Hassan'), (N'Ali'), (N'Said'), (N'Mahmoud'), (N'Ibrahim'), (N'Gamal'), (N'Fathy'), (N'Kamel'), (N'Adel'), (N'Rizk'), (N'Shalaby'), (N'El-Sayed'), (N'Abbas'), (N'Mansour'), (N'Ezzat'), (N'Tawfik'), (N'Selim'), (N'Farag'), (N'Gaber'), (N'Saleh'), (N'Amin'), (N'Nasser'), (N'Badawy');
...
INSERT INTO @CertProviders (Name) VALUES (N'Coursera'), (N'Udemy'), (N'Udacity'), (N'edX'), (N'Mahara Tech'), (N'Microsoft Learn'), (N'DataCamp');
...
-- --- Track 10 (Renewable Energy) Specific Data ---

DECLARE @RE_Positions TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(100));
INSERT INTO @RE_Positions (Name) VALUES
(N'Solar Energy Technician'), (N'Wind Turbine Technician'), (N'Renewable Energy Engineer'), (N'Energy Analyst'),
(N'Project Manager - Renewables'), (N'Grid Integration Specialist'), (N'Sustainability Consultant'), (N'Field Service Engineer'),
(N'Energy Storage Specialist'), (N'PV Designer');

DECLARE @RE_Companies TABLE (Company_ID INT);
INSERT INTO @RE_Companies (Company_ID) VALUES 
(5),  -- Elsewedy Electric
(9),  -- Orascom Construction
(19), -- Siemens
(20), -- Schneider Electric
(25); -- Hassan Allam Holding

DECLARE @RE_Certs TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(200));
INSERT INTO @RE_Certs (Name) VALUES
(N'Solar PV Design and Installation'), (N'Wind Energy Fundamentals'), (N'Certified Energy Manager (CEM)'),
(N'Smart Grid Technologies'), (N'Energy Storage Solutions'), (N'Renewable Energy Project Finance'),
(N'HOMER Pro Certified'), (N'Grid-Tied Systems Certification');

DECLARE @RE_Jobs TABLE (ID INT IDENTITY(1,1), Descr NVARCHAR(1000));
INSERT INTO @RE_Jobs (Descr) VALUES
(N'Energy efficiency audit report for a small factory.'), (N'Technical translation of solar panel installation manual.'),
(N'Market research on battery storage solutions in MENA.'), (N'Data analysis of wind speed for a potential turbine site.'),
(N'Drafting a proposal for a residential solar PV system.'), (N'Creating a financial model for a small-scale biogas plant.'),
(N'Simulating energy production using PVsyst software.'), (N'Writing a blog post on "The Future of Green Hydrogen".'),
(N'Designing a small off-grid solar system for a remote farm.'), (N'Consulting on renewable energy credits (RECs).'),
(N'Reviewing technical specifications for inverters.'), (N'Analyzing utility bill data to suggest energy savings.'),
(N'Creating a presentation on smart grid benefits.'), (N'Developing a maintenance schedule for a solar water heating system.'),
(N'Researching local regulations for grid-tied solar systems.'), (N'Load calculation for an off-grid clinic.'), (N'Sizing of a solar water pump.');
...
WHERE ibt.Track_ID = 10; -- Renewable Energy

-- --- Target Courses (Track 10) ---
...
PRINT 'Starting data generation for Track 10: Renewable Energy...';
...
BEGIN
    ...
    BEGIN
        ...
        -- Birthdate (18+ years before @CurrentDate)
        -- =================================================================
        -- !!! THIS IS THE CORRECTED LINE !!!
        -- =================================================================
        SET @Student_Birthdate = DATEADD(day, -(ABS(CHECKSUM(NEWID())) % 7300 + (18*365) + 1), GETDATE()); -- 18 to 38 years old (based on GETDATE())
        ...
        BEGIN
            ...
            SELECT TOP 1 @CertName = Name FROM @RE_Certs ORDER BY NEWID();
        ...
        BEGIN
            ...
            SELECT TOP 1 @JobDesc = Descr FROM @RE_Jobs ORDER BY NEWID();
            SET @JobEarn = (ABS(CHECKSUM(NEWID())) % 800) + 50; -- 50 to 850
        ...
        BEGIN
            ...
            IF @Branch_ID = 1 AND @Hiring_Roll < 0.80 SET @Hire = 1;  -- 80% for Smart Village
            ELSE IF @Branch_ID = 12 AND @Hiring_Roll < 0.70 SET @Hire = 1; -- 70% for Cairo Uni
            ELSE IF @Branch_ID = 11 AND @Hiring_Roll < 0.60 SET @Hire = 1; -- 60% for New Capital
            ELSE IF @Branch_ID = 2 AND @Hiring_Roll < 0.50 SET @Hire = 1;  -- 50% for Alexandria
            ELSE IF @Hiring_Roll < 0.40 SET @Hire = 1; -- 40% for all others
            ...
            BEGIN
                ...
                -- Get random company from the RE list
                SELECT TOP 1 @Company_ID = Company_ID FROM @RE_Companies ORDER BY NEWID();
                
                -- Get random position
                SELECT TOP 1 @Position = Name FROM @RE_Positions ORDER BY NEWID();

                -- Get contract type & salary (Full-Time > Part-Time)
                IF RAND() > 0.3
                BEGIN
                    SET @Contract_Type = N'Full-Time';
                    SET @Salary = (ABS(CHECKSUM(NEWID())) % 10000) + 9000; -- 9k to 19k
                END
                ELSE
                BEGIN
                    SET @Contract_Type = N'Part-Time';
                    SET @Salary = (ABS(CHECKSUM(NEWID())) % 4000) + 4000;  -- 4k to 8k
                END
