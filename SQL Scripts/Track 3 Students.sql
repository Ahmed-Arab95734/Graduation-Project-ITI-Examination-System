/*
================================================================================
SQL SCRIPT TO POPULATE STUDENT DATA FOR 'AWS Re /Start' TRACK
================================================================================
This script will:
1.  Define temporary data pools (names, addresses, faculties, etc.).
2.  Set all new primary key counters (Student, Job, Certificate) to start at 4000.
3.  Identify all 'AWS Re /Start' tracks across all intakes and branches.
...
-- B. Faculties (Weighted for AWS: CS > Eng > Others)
...
-- E. Track-Specific Data (AWS Re /Start, Track_ID = 3)
CREATE TABLE #JobPositions (Position NVARCHAR(100));
INSERT INTO #JobPositions (Position) VALUES
(N'Cloud Engineer'), (N'AWS Solutions Architect - Associate'), (N'DevOps Engineer'), 
(N'SysOps Administrator'), (N'Cloud Support Engineer'), (N'Cloud Security Specialist'), 
(N'AWS Data Engineer'), (N'Serverless Developer'), (N'Cloud Practitioner');

CREATE TABLE #Certificates (CertName NVARCHAR(200));
INSERT INTO #Certificates (CertName) VALUES
(N'AWS Certified Cloud Practitioner (CLF-C01)'),
(N'AWS Certified Solutions Architect - Associate (SAA-C03)'),
(N'AWS Certified Developer - Associate (DVA-C02)'),
(N'AWS Certified SysOps Administrator - Associate (SOA-C02)'),
(N'HashiCorp Certified: Terraform Associate'),
(N'CKA: Certified Kubernetes Administrator');

CREATE TABLE #Providers (ProviderName NVARCHAR(200));
INSERT INTO #Providers (ProviderName) VALUES
(N'Udemy'), (N'Coursera'), (N'Udacity'), (N'Amazon Web Services'), (N'DataCamp'),
(N'Microsoft'), (N'Mahara Tech'), (N'A Cloud Guru'), (N'LinkedIn Learning');

CREATE TABLE #FreelanceJobs (JobDesc NVARCHAR(1000));
INSERT INTO #FreelanceJobs (JobDesc) VALUES
(N'Migrate a WordPress site to AWS EC2/RDS'),
(N'Set up an S3 bucket with static website hosting and CloudFront'),
(N'Configure IAM roles, groups, and policies for a new team'),
(N'Write a Python Lambda function triggered by an S3 event'),
(N'Deploy a Docker container to ECS with Fargate'),
(N'Build a simple VPC with public/private subnets and NAT Gateway'),
(N'Troubleshoot a failing CloudFormation stack for a web app'),
(N'Set up AWS Backup for EC2 instances and RDS databases'),
(N'Configure CloudWatch alarms for EC2 CPU utilization'),
(N'Move on-premise data to AWS S3 using AWS DataSync'),
(N'Build a basic CI/CD pipeline with AWS CodePipeline and CodeBuild'),
(N'Set up Amazon SES for sending transactional emails'),
(N'Configure Route 53 for a new domain with health checks'),
(N'Perform a cost optimization review for a small AWS account'),
(N'Write an Ansible playbook to configure EC2 instances'),
(N'Create a serverless API using API Gateway and Lambda'),
(N'Set up AWS Organizations and basic Service Control Policies (SCPs)');
...
-- F. Relevant Companies for AWS Re /Start
...
WHERE T.Track_Name = N'AWS Re /Start'; -- Track_ID = 3
...
WHERE TC.Track_ID = 3; -- Track_ID for 'AWS Re /Start' is 3

-- ============================================================================
-- STEP 4: DECLARE COUNTERS & CURSOR
-- ============================================================================

-- Global counters starting at 4000
DECLARE @StudentIDCounter INT = 4000;
DECLARE @JobIDCounter INT = 4000;
DECLARE @CertIDCounter INT = 4000;
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @StudentAddress NVARCHAR(255) = N'789 Cloud St, ' + @StudentGov;
        ...
        BEGIN
            ...
            INSERT INTO Certificate (
                ...
                CAST(RAND() * 600 + 100 AS DECIMAL(12,2)), -- 100 to 700
                @CertDate
            );
        ...
        BEGIN
            ...
            BEGIN
                ...
                IF @ContractType = N'Full-Time'
                    SET @Salary = CAST((RAND() * 10000) + 10000 AS DECIMAL(12,2)); -- 10k to 20k
                ELSE
...
PRINT 'Successfully populated tables with 25 students for each ''AWS Re /Start'' track, starting IDs at 4000.';