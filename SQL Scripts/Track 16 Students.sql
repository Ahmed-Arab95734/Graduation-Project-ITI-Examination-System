/*
============================================================================
== SCRIPT TO POPULATE DATA FOR TRACK 16: 2D GRAPHICS DESIGN
==
== STARTING IDs: 30000
== STUDENTS PER IBT: 25
...
-- --- ID Counters ---
DECLARE @StudentID_Start INT = 30000;
DECLARE @JobID_Start INT = 30000;
DECLARE @CertID_Start INT = 30000;
...
INSERT INTO @CertProviders (Name) VALUES (N'Coursera'), (N'Udemy'), (N'Udacity'), (N'Adobe'), (N'Mahara Tech'), (N'Skillshare'), (N'Domestika');

DECLARE @FreelanceSites TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(255));
INSERT INTO @FreelanceSites (Name) VALUES (N'Upwork'), (N'Freelancer'), (N'LinkedIn'), (N'Khamsat'), (N'Mostaql'), (N'Fiverr'), (N'Behance');


-- --- Track 16 (2D Graphics Design) Specific Data ---

DECLARE @GFX_Positions TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(100));
INSERT INTO @GFX_Positions (Name) VALUES
(N'Graphic Designer'), (N'Junior Graphic Designer'), (N'Visual Designer'), (N'UI Designer'),
(N'Illustrator'), (N'Branding Specialist'), (N'Social Media Designer'), (N'Marketing Designer');

DECLARE @GFX_Companies TABLE (Company_ID INT);
INSERT INTO @GFX_Companies (Company_ID) VALUES 
(15), -- NAGWA (Educational Content Design)
(16), -- Robusta (Digital Agency)
(41), -- ArabyAds (Advertising Agency)
(46), -- Talabat (Marketing/UI Design)
(50), -- Jumia (E-commerce/Marketing Design)
(6),  -- Orange Egypt (Marketing/Brand Design)
(13), -- Etisalat by e& (Marketing/Brand Design)
(3),  -- VOIS (Internal Branding/Comms)
(40); -- Majid Al Futtaim (Retail Marketing)

DECLARE @GFX_Certs TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(200));
INSERT INTO @GFX_Certs (Name) VALUES
(N'Adobe Certified Professional in Visual Design (Photoshop)'), 
(N'Adobe Certified Professional in Graphic Design & Illustration (Illustrator)'),
(N'Graphic Design Specialization (Coursera - CalArts)'), 
(N'UI Design Fundamentals (Udemy)'),
(N'Logo Design Masterclass (Udemy)'), 
(N'Typography Fundamentals (Skillshare)'),
(N'Adobe InDesign CC - Essentials Training Course (Udemy)'), 
(N'Mahara Tech Design Principles');

DECLARE @GFX_Jobs TABLE (ID INT IDENTITY(1,1), Descr NVARCHAR(1000));
INSERT INTO @GFX_Jobs (Descr) VALUES
(N'Design a logo concept for a startup.'), (N'Create a series of social media posts for a campaign.'),
(N'Design a brochure layout for a company event.'), (N'Illustrate a character for a children''s book.'),
(N'Retouch product photos for an e-commerce website.'), (N'Design a website UI mockup in Adobe XD or Figma.'),
(N'Create vector icons for a mobile application.'), (N'Design a business card and letterhead.'),
(N'Develop a brand style guide (colors, fonts, logo usage).'), (N'Design an infographic based on provided data.'),
(N'Create illustrations for a blog post.'), (N'Design a flyer for a local business.'),
(N'Prepare print-ready files for a magazine ad.'), (N'Design email newsletter templates.'),
(N'Create custom social media cover photos and profile pictures.'), (N'Vectorize a raster logo.'),
(N'Design packaging for a new product.');
...
WHERE ibt.Track_ID = 16; -- 2D Graphics Design

-- --- Target Courses (Track 16) ---
...
PRINT 'Starting data generation for Track 16: 2D Graphics Design...';
...
BEGIN
    ...
    BEGIN
        ...
        -- Faculty (Weighted - User requested CS/Eng priority)
        ...
        VALUES
            ...
            -- Adding Instagram is relevant for graphics designers
        IF RAND() > 0.3 
            INSERT INTO Student_Social (Student_ID, Social_Type, Social_Url)
            VALUES (@StudentID, N'Instagram', N'https://instagram.com/' + @SocialName);
        -- Behance/Dribbble would be ideal, but not in CHECK constraint. Add Facebook sometimes.
        IF RAND() > 0.6
             INSERT INTO Student_Social (Student_ID, Social_Type, Social_Url)
        ...
        BEGIN
            ...
            SELECT TOP 1 @CertName = Name FROM @GFX_Certs ORDER BY NEWID();
            SELECT TOP 1 @CertProvider = Name FROM @CertProviders ORDER BY NEWID();
            -- Cert cost might vary, Adobe certs can be pricier
            SET @CertCost = CASE WHEN @CertProvider = 'Adobe' THEN (ABS(CHECKSUM(NEWID())) % 100 + 100)*10 ELSE (ABS(CHECKSUM(NEWID())) % 150) * 10 END; 
        ...
        DECLARE @JobCount INT = ABS(CHECKSUM(NEWID())) % 7; -- 0 to 6 jobs (graphic design often has more small gigs)
        DECLARE @j INT = 0;
        WHILE @j < @JobCount
        BEGIN
            ...
            SELECT TOP 1 @JobDesc = Descr FROM @GFX_Jobs ORDER BY NEWID();
            SET @JobEarn = (ABS(CHECKSUM(NEWID())) % 500) + 40; -- 40 to 540 (smaller design gigs)
        ...
        BEGIN
            ...
            IF @Branch_ID = 1 AND @Hiring_Roll < 0.70 SET @Hire = 1;  -- 70% for Smart Village
            ...
            BEGIN
                ...
                -- Get random company from the GFX list
                SELECT TOP 1 @Company_ID = Company_ID FROM @GFX_Companies ORDER BY NEWID();
                
                -- Get random position
                SELECT TOP 1 @Position = Name FROM @GFX_Positions ORDER BY NEWID();

                -- Get contract type & salary (Full-Time > Part-Time)
                IF RAND() > 0.35
                BEGIN
                    SET @Contract_Type = N'Full-Time';
                    SET @Salary = (ABS(CHECKSUM(NEWID())) % 7000) + 6000; -- 6k to 13k (Starting graphic design roles)
                END
                ELSE
                BEGIN
                    SET @Contract_Type = N'Part-Time';
                    SET @Salary = (ABS(CHECKSUM(NEWID())) % 2500) + 2500;  -- 2.5k to 5k
                END
...
PRINT 'Data generation complete for Track 16.';
