/********************************************************************************
* RUN SCRIPT
* Description: Populates Student and related tables for Track 'Salesforce Specialist' (ID 24)
* Start IDs: 46000 for Student, Job, and Certificate
* Students per IBT: 25
*
* Rules Enforced:
* 1.  Only for Track_ID 24.
* 2.  25 Students per Intake_Branch_Track (IBT).
* 3.  Unique IDs starting from 46000.
...
* 9.  Faculties weighted towards 'Computers Sciences' and 'Engineering'. Also considers Commerce/Business Admin for SF.
...
* 16. Positions, Companies, Certs, Freelance jobs are all relevant to 'Salesforce Specialist'.
* 17. Certs/Freelance jobs dated *during* the intake period.
* 18. Student_Course table populated sequentially for all 12 'Salesforce Specialist' courses.
...
-- -- -- -- -- -- -- -- -- -- -- --
-- Faculties (Weighted) - Adjusted slightly for Salesforce
-- -- -- -- -- -- -- -- -- -- -- --
DECLARE @OtherFaculties TABLE (Name NVARCHAR(100));
INSERT INTO @OtherFaculties VALUES
(N'Faculty of Science'), (N'Faculty of Information Systems'), (N'Faculty of Education'),
(N'Faculty of Economics and Political Science'), (N'Faculty of Agriculture'), (N'Faculty of Fine Arts'),
(N'Faculty of Applied Arts'), (N'Faculty of Arts'); -- Removed Commerce/Business as they are now weighted

-- -- -- -- -- -- -- -- -- -- -- --
-- Track-Specific Data: Salesforce Specialist (Track 24)
-- -- -- -- -- -- -- -- -- -- -- --
...
FROM Track_Course WHERE Track_ID = 24;

DECLARE @Companies_SF TABLE (ID INT);
INSERT INTO @Companies_SF VALUES
(3), (4), (6), (8), (10), (11), (13), (14), (21), (22), (23), (24), (27), (28), (29), (30), (31), (32), (40), (47), (48); -- VOIS, Accenture, Orange, CIB, IBM, Capgemini, Etisalat, Fawry, NBE, Banque Misr, WE, Raya, PwC, Deloitte, EY, KPMG, Oracle, SAP, Majid Al Futtaim, Kyndryl, Atos

DECLARE @Positions_SF TABLE (Name NVARCHAR(100));
INSERT INTO @Positions_SF VALUES
(N'Salesforce Administrator'), (N'Salesforce Consultant'), (N'Salesforce Business Analyst'), (N'Junior Salesforce Developer'),
(N'Salesforce Support Specialist'), (N'CRM Specialist'), (N'Salesforce Analyst'), (N'Salesforce Platform App Builder');

DECLARE @CertProviders TABLE (Name NVARCHAR(200));
INSERT INTO @CertProviders VALUES (N'Udemy'), (N'Coursera'), (N'Udacity'), (N'Mahara Tech'), (N'DataCamp'), (N'Microsoft'), (N'Salesforce Trailhead'); -- Added Trailhead

DECLARE @CertNames_SF TABLE (Name NVARCHAR(200));
INSERT INTO @CertNames_SF VALUES
(N'Salesforce Certified Administrator Prep'), (N'Salesforce Platform App Builder Prep'), (N'Salesforce Sales Cloud Consultant Prep'),
(N'Salesforce Service Cloud Consultant Prep'), (N'Introduction to Apex Programming'), (N'CRM Fundamentals');
...
DECLARE @FreelanceJobs_SF TABLE (Name NVARCHAR(1000));
INSERT INTO @FreelanceJobs_SF VALUES
(N'Created custom fields and objects in Salesforce.'), (N'Built a validation rule for lead conversion.'),
(N'Designed a Salesforce Flow to automate case assignment.'), (N'Generated a sales performance report and dashboard.'),
(N'Configured user profiles and permission sets.'), (N'Imported customer data using Data Loader.'),
(N'Customized page layouts for the Account object.'), (N'Set up email templates for marketing campaigns.'),
(N'Troubleshot login issues for Salesforce users.'), (N'Created a simple Lightning Web Component.'),
(N'Configured Salesforce Mobile app settings.'), (N'Built a custom report type for opportunity analysis.'),
(N'Assisted with setting up a Salesforce Community page.'), (N'Managed user roles and hierarchies.'),
(N'Created workflow rules for task automation.');


-- =============================================================================
-- 2. DECLARE CURSOR AND LOOP VARIABLES
-- =============================================================================
DECLARE @CurrentID INT = 46000;
DECLARE @CurrentJobID INT = 46000;
DECLARE @CurrentCertID INT = 46000;
...
DECLARE IBT_Cursor CURSOR FOR
    ...
    WHERE
        IBT.Track_ID = 24; -- <<<<<<< ONLY 'Salesforce Specialist'
...
BEGIN
    ...
    BEGIN
        ...
        -- -- -- -- -- -- -- -- -- -- -- --
        -- 4.3. Generate Weighted Faculty (Adjusted for SF)
        -- -- -- -- -- -- -- -- -- -- -- --
        SET @FacultyRand = RAND();
        IF @FacultyRand < 0.30 -- 30% chance CS
            SET @Faculty = N'Faculty of Computers Sciences';
        ELSE IF @FacultyRand < 0.55 -- 25% chance Eng (total 55%)
            SET @Faculty = N'Faculty of Engineering';
        ELSE IF @FacultyRand < 0.75 -- 20% chance Commerce/Business (total 75%)
            SET @Faculty = CASE WHEN RAND() > 0.5 THEN N'Faculty of Commerce' ELSE N'Faculty of Business Administration' END;
        ELSE -- 25% chance Others
      ...
      --(@StudentID, N'GitHub', N'https://github.com/' + LOWER(REPLACE(@Fname, ' ', '')) + LOWER(REPLACE(@Lname, ' ', '')) + CAST(@StudentID % 1000 AS VARCHAR(4))), -- Less relevant for SF Admin/Consultant focus
        ...
        BEGIN
            ...
            SET @CertName = (SELECT TOP 1 Name FROM @CertNames_SF ORDER BY NEWID());
            SET @CertProvider = (SELECT TOP 1 Name FROM @CertProviders ORDER BY NEWID());
            SET @CertCost = CAST(RAND() * 500 AS DECIMAL(12,2)); -- SF Certs often cost around $200-$400
        ...
        BEGIN
            ...
            SET @JobDesc = (SELECT TOP 1 Name FROM @FreelanceJobs_SF ORDER BY NEWID());
        ...
        BEGIN
            SET @CompanyID = (SELECT TOP 1 ID FROM @Companies_SF ORDER BY NEWID());
            SET @Position = (SELECT TOP 1 Name FROM @Positions_SF ORDER BY NEWID());
            SET @Contract = CASE WHEN RAND() > 0.7 THEN N'Full-Time' ELSE N'Part-Time' END;
            
            -- Full-Time (9k-18k) > Part-Time (5k-9k)
            SET @Salary = CASE @Contract
                WHEN N'Full-Time' THEN CAST(RAND() * 9000 + 9000 AS DECIMAL(12,2))
                ELSE CAST(RAND() * 4000 + 5000 AS DECIMAL(12,2))
            END;
...
PRINT 'Successfully populated database for Track 24 (Salesforce Specialist).';
PRINT 'Total Students Inserted: ' + CAST((@CurrentID - 46000) AS VARCHAR(10));
