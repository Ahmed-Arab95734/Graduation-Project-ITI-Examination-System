/*
================================================================================
SQL SCRIPT TO POPULATE STUDENT DATA FOR 'Data Engineering' TRACK
================================================================================
This script will:
1.  Define temporary data pools (names, addresses, faculties, etc.).
2.  Set all new primary key counters (Student, Job, Certificate) to start at 8000.
3.  Identify all 'Data Engineering' tracks across all intakes and branches.
4.  Loop through each of these tracks and generate 25 students for each.
5.  Apply all business logic:
    - Status ('Graduated' vs 'In Progress') based on '2025-10-19'.
    - Weighted faculty selection (Computer Science > Engineering > Others).
    ...
    - Generation of relevant certificates, freelance jobs, and course enrollments.
================================================================================
*/
SET NOCOUNT ON;
...
-- B. Faculties (Weighted for Data Engineering: CS > Eng > Others)
...
-- E. Track-Specific Data (Data Engineering, Track_ID = 5)
CREATE TABLE #JobPositions (Position NVARCHAR(100));
INSERT INTO #JobPositions (Position) VALUES
(N'Data Engineer'), (N'Big Data Engineer'), (N'ETL Developer'), 
(N'Data Warehouse Engineer'), (N'Cloud Data Engineer'), (N'BI Engineer'), 
(N'Data Platform Engineer'), (N'Analytics Engineer'), (N'Database Developer');

CREATE TABLE #Certificates (CertName NVARCHAR(200));
INSERT INTO #Certificates (CertName) VALUES
(N'AWS Certified Data Analytics - Specialty'),
(N'Google Cloud Certified - Professional Data Engineer'),
(N'Microsoft Certified: Azure Data Engineer Associate (DP-203)'),
(N'Databricks Certified Associate Developer for Apache Spark'),
(N'Cloudera Certified Associate (CCA) Spark and Hadoop Developer'),
(N'Snowflake SnowPro Core Certification');
...
(N'Amazon Web Services'), (N'Google Cloud'), (N'Databricks'), (N'Cloudera'), (N'Snowflake');

CREATE TABLE #FreelanceJobs (JobDesc NVARCHAR(1000));
INSERT INTO #FreelanceJobs (JobDesc) VALUES
(N'Build an ETL pipeline using Python and Airflow to load data into Redshift.'),
(N'Optimize SQL queries for a large PostgreSQL data warehouse.'),
(N'Set up a Kafka cluster for real-time data streaming.'),
(N'Develop Spark jobs for batch processing large datasets on EMR.'),
(N'Design and implement a data model for a new data warehouse on BigQuery.'),
(N'Migrate an on-premise SQL Server database to Azure SQL Database.'),
(N'Build data quality checks using Great Expectations.'),
(N'Set up data ingestion from various APIs into a data lake (S3/ADLS).'),
(N'Develop data transformations using dbt (data build tool).'),
(N'Configure and manage permissions for a Snowflake data warehouse.'),
(N'Create a dashboard using Looker Studio connected to BigQuery.'),
(N'Optimize Spark application performance.'),
(N'Implement CDC (Change Data Capture) using Debezium.'),
(N'Write Python scripts for data cleaning and validation.'),
(N'Set up monitoring and alerting for data pipelines using Datadog.'),
(N'Consult on choosing the right cloud data warehouse technology.');
...
-- F. Relevant Companies for Data Engineering
CREATE TABLE #RelevantCompanies (Company_ID INT);
INSERT INTO #RelevantCompanies (Company_ID) VALUES
(1),  -- Microsoft
(2),  -- Amazon (AWS)
(3),  -- Vodafone Intelligent Solutions (VOIS)
(4),  -- Accenture
(6),  -- Orange Egypt
(8),  -- CIB (Commercial International Bank)
(10), -- IBM
(11), -- Capgemini
(13), -- Etisalat by e&
(14), -- Fawry
(18), -- Google
(21), -- National Bank of Egypt (NBE)
(22), -- Banque Misr
(23), -- Telecom Egypt (WE)
(31), -- Oracle
(32), -- SAP
...
WHERE T.Track_Name = N'Data Engineering'; -- Track_ID = 5
...
WHERE TC.Track_ID = 5; -- Track_ID for 'Data Engineering' is 5

-- ============================================================================
-- STEP 4: DECLARE COUNTERS & CURSOR
-- ============================================================================

-- Global counters starting at 8000
DECLARE @StudentIDCounter INT = 8000;
DECLARE @JobIDCounter INT = 8000;
DECLARE @CertIDCounter INT = 8000;
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @StudentAddress NVARCHAR(255); -- Changed from NVARCHAR(255)
        ...
        BEGIN
            ...
            -- Prevent course end date exceeding intake end date if duration calculation causes overflow on last item
            ...
            SET @CourseLoopDate = DATEADD(DAY, 1, @CourseEndDate); -- Next course starts the day after
             -- Ensure next start date doesn't exceed end date
            IF @CourseLoopDate > @EndDate SET @CourseLoopDate = @EndDate;
        ...
        DECLARE @NumCerts INT = 1 + (ABS(CHECKSUM(NEWID())) % 4); -- 1 to 4 certs
        DECLARE @CertCounter INT = 0;
        WHILE @CertCounter < @NumCerts
        BEGIN
            DECLARE @CertDate DATE = DATEADD(DAY, ABS(CHECKSUM(NEWID())) % (@TotalDays + 1), @StartDate); -- Include start/end date
            IF @TotalDays = 0 SET @CertDate = @StartDate; -- Handle zero duration case
            ...
            INSERT INTO Certificate (
                ...
                CAST(RAND() * 700 + 100 AS DECIMAL(12,2)), -- 100 to 800
                @CertDate
            );
        ...
        DECLARE @NumJobs INT = (ABS(CHECKSUM(NEWID())) % 6); -- 0 to 5 jobs
        DECLARE @JobCounter INT = 0;
        WHILE @JobCounter < @NumJobs
        BEGIN
             DECLARE @JobDate DATE = DATEADD(DAY, ABS(CHECKSUM(NEWID())) % (@TotalDays + 1), @StartDate); -- Include start/end date
             IF @TotalDays = 0 SET @JobDate = @StartDate; -- Handle zero duration case

            INSERT INTO Freelance_Job (
                Job_ID, Student_ID, Job_Earn, Job_Date, Job_Site, Description
            ) VALUES (
                @JobIDCounter, @StudentIDCounter,
                CAST((RAND() * 1200) + 150 AS DECIMAL(12,2)), -- 150 to 1350
                @JobDate,
        ...
        BEGIN
            ...
            DECLARE @HireChance FLOAT = 0.35; -- Base chance slightly higher for DE
            IF @BranchName = N'Smart Village' SET @HireChance = 0.75;
            ELSE IF @BranchName = N'Cairo University' SET @HireChance = 0.65;
            ELSE IF @BranchName = N'New Capital' SET @HireChance = 0.55;
            ELSE IF @BranchName = N'Alexandria' SET @HireChance = 0.45;

            IF RAND() < @HireChance
            BEGIN
                DECLARE @ContractType NVARCHAR(50) = IIF(RAND() < 0.9, N'Full-Time', N'Part-Time'); -- 90% Full-Time for DE
                DECLARE @Salary DECIMAL(12,2);
                
                IF @ContractType = N'Full-Time'
                    SET @Salary = CAST((RAND() * 13000) + 15000 AS DECIMAL(12,2)); -- 15k to 28k
                ELSE
                    SET @Salary = CAST((RAND() * 6000) + 7000 AS DECIMAL(12,2)); -- 7k to 13k

                DECLARE @Position NVARCHAR(100) = (SELECT TOP 1 Position FROM #JobPositions ORDER BY NEWID());
                -- Select from relevant companies
...
PRINT 'Successfully populated tables with 25 students for each ''Data Engineering'' track, starting IDs at 8000.';