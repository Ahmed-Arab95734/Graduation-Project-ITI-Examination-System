/*
================================================================================
SQL SCRIPT TO POPULATE STUDENT DATA FOR 'Software Testing' TRACK
================================================================================
This script will:
1.  Define temporary data pools (names, addresses, faculties, etc.).
2.  Set all new primary key counters (Student, Job, Certificate) to start at 16000.
3.  Identify all 'Software Testing' tracks.
...
-- B. Faculties (Weighted for Software Testing: CS > Eng > Others)
...
-- E. Track-Specific Data (Software Testing, Track_ID = 9)
CREATE TABLE #JobPositions (Position NVARCHAR(100));
INSERT INTO #JobPositions (Position) VALUES
(N'Software Tester'), (N'QA Engineer'), (N'Test Automation Engineer'), 
(N'Quality Assurance Analyst'), (N'Manual Tester'), (N'Performance Tester'), 
(N'API Tester'), (N'Junior QA Specialist');

CREATE TABLE #Certificates (CertName NVARCHAR(200));
INSERT INTO #Certificates (CertName) VALUES
(N'ISTQB Certified Tester Foundation Level (CTFL)'),
(N'ISTQB Certified Tester Advanced Level - Test Automation Engineer (CTAL-TAE)'),
(N'Selenium WebDriver with Java - Basics to Advanced'),
(N'API Testing with Postman'),
(N'Performance Testing using JMeter'),
(N'Agile Testing Fundamentals');
...
(N'ISTQB (via accredited providers like EG-TEST)'), (N'LinkedIn Learning'), (N'Pluralsight');

CREATE TABLE #FreelanceJobs (JobDesc NVARCHAR(1000));
INSERT INTO #FreelanceJobs (JobDesc) VALUES
(N'Perform manual functional testing for a new web application feature.'),
(N'Write automated UI tests using Selenium WebDriver for a login page.'),
(N'Execute API tests using Postman collections provided by the client.'),
(N'Create a basic performance test script in JMeter for a specific endpoint.'),
(N'Write detailed bug reports with steps to reproduce for identified issues.'),
(N'Develop a test plan outline for testing a small mobile app.'),
(N'Perform exploratory testing on a website to find usability issues.'),
(N'Write test cases based on user stories or requirements documents.'),
(N'Set up a basic Selenium Grid for parallel test execution.'),
(N'Test website responsiveness across different browser sizes and devices.'),
(N'Perform regression testing after bug fixes have been deployed.'),
(N'Write automated tests for API endpoints using Python Requests/Pytest.'),
(N'Create end-to-end test scenarios for a user workflow.'),
(N'Test accessibility features according to WCAG guidelines.'),
(N'Review existing test cases for clarity and coverage.'),
(N'Manually test database queries and data integrity.');
...
-- F. Relevant Companies for Software Testing (Broad applicability)
CREATE TABLE #RelevantCompanies (Company_ID INT);
-- Most tech companies, consultancies, banks, and large national companies hire testers.
...
WHERE T.Track_Name = N'Software Testing'; -- Track_ID = 9
...
WHERE TC.Track_ID = 9; -- Track_ID for 'Software Testing' is 9

-- ============================================================================
-- STEP 4: DECLARE COUNTERS & CURSOR
-- ============================================================================

-- Global counters starting at 16000
DECLARE @StudentIDCounter INT = 16000;
DECLARE @JobIDCounter INT = 16000;
DECLARE @CertIDCounter INT = 16000;
...
BEGIN
    ...
    BEGIN
        ...
        (@StudentIDCounter, N'GitHub', N'https://github.com/' + @SocialNameTag), -- Testers might still use GitHub
        ...
        DECLARE @NumJobs INT = (ABS(CHECKSUM(NEWID())) % 5); -- 0 to 4 freelance jobs
        ...
        BEGIN
            ...
            INSERT INTO Freelance_Job (
                ...
                CAST((RAND() * 500) + 75 AS DECIMAL(12,2)), -- 75 to 575
                @JobDate,
        ...
        BEGIN
            ...
            BEGIN
                ...
                IF @ContractType = N'Full-Time'
                    SET @Salary = CAST((RAND() * 8000) + 8000 AS DECIMAL(12,2)); -- 8k to 16k
                ELSE
                    SET @Salary = CAST((RAND() * 3500) + 4000 AS DECIMAL(12,2)); -- 4k to 7.5k

                DECLARE @Position NVARCHAR(100) = (SELECT TOP 1 Position FROM #JobPositions ORDER BY NEWID());
                -- Select from relevant companies (very broad for testing)
...
PRINT 'Successfully populated tables with 25 students for each ''Software Testing'' track, starting IDs at 16000.';