/*
================================================================================
SQL SCRIPT TO POPULATE STUDENT DATA FOR 'Industrial Automation' TRACK
================================================================================
This script will:
1.  Define temporary data pools (names, addresses, faculties, etc.).
2.  Set all new primary key counters (Student, Job, Certificate) to start at 2000.
3.  Identify all 'Industrial Automation' tracks across all intakes and branches.
...
5.  Apply all business logic:
    ...
    - Weighted faculty selection (Engineering > Computer Science > Others).
...
-- B. Faculties (Weighted for Industrial Automation: Eng > CS > Others)
CREATE TABLE #Faculties (FacultyName NVARCHAR(100));
INSERT INTO #Faculties (FacultyName) VALUES
(N'Faculty of Engineering'), (N'Faculty of Engineering'), (N'Faculty of Engineering'), (N'Faculty of Engineering'), (N'Faculty of Engineering'), -- 5 (Most)
(N'Faculty of Computers Sciences'), (N'Faculty of Computers Sciences'), (N'Faculty of Computers Sciences'), (N'Faculty of Computers Sciences'), -- 4 (Second)
(N'Faculty of Science'), 
(N'Faculty of Commerce'), 
(N'Faculty of Business Administration'), 
(N'Faculty of Information Systems'), 
(N'Faculty of Education'), 
(N'Faculty of Economics and Political Science'), 
(N'Faculty of Agriculture'), 
(N'Faculty of Fine Arts'), 
(N'Faculty of Applied Arts'), 
(N'Faculty of Arts'); -- 10 Others
...
-- E. Track-Specific Data (Industrial Automation, Track_ID = 2)
CREATE TABLE #JobPositions (Position NVARCHAR(100));
INSERT INTO #JobPositions (Position) VALUES
(N'Automation Engineer'), (N'Controls Engineer'), (N'PLC/SCADA Programmer'), 
(N'Mechatronics Engineer'), (N'Robotics Engineer'), (N'Industrial IT Specialist'), 
(N'Control Systems Engineer'), (N'Instrumentation Engineer');

CREATE TABLE #Certificates (CertName NVARCHAR(200));
INSERT INTO #Certificates (CertName) VALUES
(N'Siemens TIA Portal Certified Programmer'),
(N'Rockwell Automation Certified'),
(N'Certified SCADA Professional (CSP)'),
(N'Intro to Industrial IoT (IIoT)'),
(N'Functional Safety Engineer (T?V Rheinland)'),
(N'Certified Robotics Technician (CRT)');

CREATE TABLE #Providers (ProviderName NVARCHAR(200));
INSERT INTO #Providers (ProviderName) VALUES
(N'Udemy'), (N'Coursera'), (N'Udacity'), (N'Siemens'), (N'Rockwell Automation'), 
(N'Mahara Tech'), (N'LinkedIn Learning'), (N'T?V Rheinland');

CREATE TABLE #FreelanceJobs (JobDesc NVARCHAR(1000));
INSERT INTO #FreelanceJobs (JobDesc) VALUES
(N'Develop PLC ladder logic for a small conveyor system.'),
(N'Design an HMI dashboard for process monitoring.'),
(N'Troubleshoot and debug an existing PLC program remotely.'),
(N'Program a 6-axis robot arm for a pick-and-place task.'),
(N'Consult on component selection for an automation project.'),
(N'Create electrical drawings for a control panel.'),
(N'Integrate a VFD (Variable Frequency Drive) with a PLC.'),
(N'Set up Modbus TCP/IP communication between devices.'),
(N'Develop a SCADA system for a small water treatment plant.'),
(N'Write documentation for an automated machine.'),
(N'Configure industrial network switches (EtherNet/IP).'),
(N'Simulate a PLC program using factory simulation software.'),
(N'Upgrade an old Siemens S7-300 project to TIA Portal.'),
(N'Create a parts list (BOM) for an automation cell.'),
(N'Develop a maintenance schedule for robotic equipment.');
...
-- F. Relevant Companies for Industrial Automation
...
-- Pre-load phones from the first script (if run in same session)
-- This is a safeguard; for a clean run, this is empty.
-- If you are running this *after* the Power BI script, you must also load its phones.
-- For this script to be standalone, we assume it's clean.
...
WHERE T.Track_Name = N'Industrial Automation'; -- Track_ID = 2
...
WHERE TC.Track_ID = 2; -- Track_ID for 'Industrial Automation' is 2

-- ============================================================================
-- STEP 4: DECLARE COUNTERS & CURSOR
-- ============================================================================

-- Global counters starting at 2000
DECLARE @StudentIDCounter INT = 2000;
DECLARE @JobIDCounter INT = 2000;
DECLARE @CertIDCounter INT = 2000;
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @StudentAddress NVARCHAR(255) = N'456 Industry St, ' + @StudentGov;

        -- 3. Generate Faculty (Weighted Eng > CS > Others)
        ...
        BEGIN
            ...
            INSERT INTO Certificate (
                ...
                CAST(RAND() * 800 + 100 AS DECIMAL(12,2)), -- 100 to 900
                @CertDate
            );
        ...
        DECLARE @NumJobs INT = (ABS(CHECKSUM(NEWID())) % 4); -- 0 to 3 jobs
        ...
        BEGIN
            ...
            INSERT INTO Freelance_Job (
                ...
                CAST((RAND() * 1000) + 100 AS DECIMAL(12,2)), -- 100 to 1100
                @JobDate,
        ...
        BEGIN
            ...
            IF RAND() < @HireChance
            BEGIN
                DECLARE @ContractType NVARCHAR(50) = IIF(RAND() < 0.8, N'Full-Time', N'Part-Time'); -- 80% Full-Time
                DECLARE @Salary DECIMAL(12,2);
                
                IF @ContractType = N'Full-Time'
                    SET @Salary = CAST((RAND() * 9000) + 9000 AS DECIMAL(12,2)); -- 9k to 18k
                ELSE
                    SET @Salary = CAST((RAND() * 4000) + 4000 AS DECIMAL(12,2)); -- 4k to 8k
...
PRINT 'Successfully populated tables with 25 students for each ''Industrial Automation'' track, starting IDs at 2000.';