/*
================================================================================
SQL SCRIPT TO POPULATE STUDENT DATA FOR 'Front-End and Cross Platform Mobile Development' TRACK
================================================================================
This script will:
1.  Define temporary data pools (names, addresses, faculties, etc.).
2.  Set all new primary key counters (Student, Job, Certificate) to start at 10000.
3.  Identify all 'Front-End and Cross Platform Mobile Development' tracks.
...
-- B. Faculties (Weighted for Front-End/Mobile: CS > Eng > Others)
...
-- E. Track-Specific Data (Front-End and Cross Platform Mobile Dev, Track_ID = 6)
CREATE TABLE #JobPositions (Position NVARCHAR(100));
INSERT INTO #JobPositions (Position) VALUES
(N'Front-End Developer'), (N'React Developer'), (N'React Native Developer'), 
(N'Flutter Developer'), (N'Mobile App Developer'), (N'UI Developer'), 
(N'JavaScript Engineer'), (N'Cross-Platform Developer'), (N'Web Developer');

CREATE TABLE #Certificates (CertName NVARCHAR(200));
INSERT INTO #Certificates (CertName) VALUES
(N'Meta Front-End Developer Professional Certificate'),
(N'Google UX Design Professional Certificate'),
(N'React - The Complete Guide (incl Hooks, React Router, Redux)'),
(N'Flutter & Dart - The Complete Guide'),
(N'React Native - The Practical Guide'),
(N'Advanced JavaScript Concepts'),
(N'Complete Web & Mobile Designer: UI/UX, Figma');
...
(N'Build a responsive landing page from a Figma design using HTML/CSS/JS.'),
(N'Develop a reusable React component library for a web app.'),
(N'Create several screens for a Flutter mobile application based on UI mockups.'),
(N'Fix CSS layout bugs on an existing website across multiple browsers.'),
(N'Integrate a REST API into a React Native application to display data.'),
(N'Build a complete responsive website for a small business using React.'),
(N'Optimize the performance (loading speed) of a front-end application.'),
(N'Develop a Progressive Web App (PWA) version of an existing website.'),
(N'Design and implement a user interface for a mobile app screen.'),
(N'Add a new feature (e.g., user authentication) to an existing React Native app.'),
(N'Build an interactive form with complex validation using JavaScript.'),
(N'Create interactive animations using CSS and JavaScript.'),
(N'Debug and fix JavaScript errors reported in the browser console.'),
(N'Clone the UI of a popular website (e.g., Twitter feed) using React.'),
(N'Build a personal portfolio website using modern front-end technologies.'),
(N'Convert a static HTML/CSS template into a dynamic React application.');
...
-- F. Relevant Companies for Front-End/Mobile
...
WHERE T.Track_Name = N'Front-End and Cross Platform Mobile Development'; -- Track_ID = 6
...
WHERE TC.Track_ID = 6; -- Track_ID for 'Front-End and Cross Platform Mobile Development' is 6

-- ============================================================================
-- STEP 4: DECLARE COUNTERS & CURSOR
-- ============================================================================

-- Global counters starting at 10000
DECLARE @StudentIDCounter INT = 10000;
DECLARE @JobIDCounter INT = 10000;
DECLARE @CertIDCounter INT = 10000;
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @StudentAddress NVARCHAR(255); -- Changed to store just governorate
        ...
        DECLARE @CourseCount INT = (SELECT COUNT(*) FROM #TrackCourses);
         -- Avoid division by zero if no courses found for the track
        ...
        WHILE @@FETCH_STATUS = 0 AND @CourseLoopDate <= @EndDate
        BEGIN
        ...
        BEGIN
             ...
             IF @TotalDays > 0 -- Ensure TotalDays is not negative or zero before using modulo
            ...
            INSERT INTO Certificate (
                ...
                CAST(RAND() * 400 + 50 AS DECIMAL(12,2)), -- 50 to 450
                @CertDate
            );
        ...
        BEGIN
            ...
            BEGIN
                ...
                IF @ContractType = N'Full-Time'
                    SET @Salary = CAST((RAND() * 9000) + 10000 AS DECIMAL(12,2)); -- 10k to 19k
                ELSE
                    SET @Salary = CAST((RAND() * 4000) + 5000 AS DECIMAL(12,2)); -- 5k to 9k
...
PRINT 'Successfully populated tables with 25 students for each ''Front-End and Cross Platform Mobile Development'' track, starting IDs at 10000.';