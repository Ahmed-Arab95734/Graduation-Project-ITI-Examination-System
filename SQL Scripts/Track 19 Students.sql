/********************************************************************************
* RUN SCRIPT
* Description: Populates Student and related tables for Track 'Concept Art' (ID 19)
* Start IDs: 36000 for Student, Job, and Certificate
* Students per IBT: 25
*
* Rules Enforced:
* 1.  Only for Track_ID 19.
* 2.  25 Students per Intake_Branch_Track (IBT).
* 3.  Unique IDs starting from 36000.
...
* 16. Positions, Companies, Certs, Freelance jobs are all relevant to 'Concept Art'.
* 17. Certs/Freelance jobs dated *during* the intake period.
* 18. Student_Course table populated sequentially for all 12 'Concept Art' courses.
...
(N'Faculty of Computers Sciences'), (N'Faculty of Engineering'), (N'Faculty of Science'),
(N'Faculty of Commerce'), (N'Faculty of Business Administration'), (N'Faculty of Information Systems'),
(N'Faculty of Education'), (N'Faculty of Economics and Political Science'), (N'Faculty of Agriculture');

DECLARE @OtherFaculties TABLE (Name NVARCHAR(100));
INSERT INTO @OtherFaculties VALUES
(N'Faculty of Fine Arts'), (N'Faculty of Applied Arts'), (N'Faculty of Arts'), (N'Faculty of Science'),
(N'Faculty of Commerce'), (N'Faculty of Business Administration'), (N'Faculty of Information Systems'),
(N'Faculty of Education'), (N'Faculty of Economics and Political Science'), (N'Faculty of Agriculture');

-- -- -- -- -- -- -- -- -- -- -- --
-- Track-Specific Data: Concept Art (Track 19)
-- -- -- -- -- -- -- -- -- -- -- --
...
FROM Track_Course WHERE Track_ID = 19;

DECLARE @Companies_CA TABLE (ID INT);
INSERT INTO @Companies_CA VALUES (15), (16), (41), (46), (50), (3), (6); -- NAGWA, Robusta, ArabyAds, Talabat, Jumia, VOIS, Orange

DECLARE @Positions_CA TABLE (Name NVARCHAR(100));
INSERT INTO @Positions_CA VALUES
(N'Concept Artist'), (N'Character Designer'), (N'Environment Artist'), (N'Prop Designer'),
(N'2D Artist'), (N'Illustrator'), (N'Visual Development Artist'), (N'UI Artist'), (N'Texture Artist');
...
DECLARE @CertNames_CA TABLE (Name NVARCHAR(200));
INSERT INTO @CertNames_CA VALUES
(N'Digital Painting Masterclass'), (N'Character Design for Beginners'), (N'Environment Design with Photoshop'),
(N'The Ultimate Drawing Course'), (N'Concept Art for Video Games'), (N'Anatomy for Artists');
...
DECLARE @FreelanceJobs_CA TABLE (Name NVARCHAR(1000));
INSERT INTO @FreelanceJobs_CA VALUES
(N'Designed character concepts for an indie mobile game.'), (N'Created environment mood paintings for a short film.'),
(N'Illustrated a series of props for a fantasy world.'), (N'Painted promotional art for a new product launch.'),
(N'Designed creature concepts for a TTRPG module.'), (N'Created 2D assets for a platformer game.'),
(N'Storyboarded a short animated commercial.'), (N'Designed vehicle concepts for a sci-fi project.'),
(N'Illustrated splash art for a game update.'), (N'Created detailed prop callout sheets for 3D modelers.'),
(N'Painted landscape concepts for a client pitch.'), (N'Designed a set of 10 unique fantasy weapons.'),
(N'Sketched character turnaround sheets.'), (N'Created concept art for a marketing campaign.'),
(N'Designed UI elements and icons for a mobile app.');


-- =============================================================================
-- 2. DECLARE CURSOR AND LOOP VARIABLES
-- =============================================================================
DECLARE @CurrentID INT = 36000;
DECLARE @CurrentJobID INT = 36000;
DECLARE @CurrentCertID INT = 36000;
...
DECLARE IBT_Cursor CURSOR FOR
    ...
    WHERE
        IBT.Track_ID = 19; -- <<<<<<< ONLY 'Concept Art'
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET @CertName = (SELECT TOP 1 Name FROM @CertNames_CA ORDER BY NEWID());
        ...
        BEGIN
            ...
            SET @JobDesc = (SELECT TOP 1 Name FROM @FreelanceJobs_CA ORDER BY NEWID());
        ...
        BEGIN
            SET @CompanyID = (SELECT TOP 1 ID FROM @Companies_CA ORDER BY NEWID());
            SET @Position = (SELECT TOP 1 Name FROM @Positions_CA ORDER BY NEWID());
            SET @Contract = CASE WHEN RAND() > 0.7 THEN N'Full-Time' ELSE N'Part-Time' END;
            
            -- Full-Time (7k-15k) > Part-Time (3k-7k)
            SET @Salary = CASE @Contract
                WHEN N'Full-Time' THEN CAST(RAND() * 8000 + 7000 AS DECIMAL(12,2))
                ELSE CAST(RAND() * 4000 + 3000 AS DECIMAL(12,2))
            END;
...
PRINT 'Successfully populated database for Track 19 (Concept Art).';
PRINT 'Total Students Inserted: ' + CAST((@CurrentID - 36000) AS VARCHAR(10));
