/*
============================================================================
== SCRIPT TO POPULATE DATA FOR TRACK 15: FULL STACK WEB DEVELOPMENT USING PHP
==
== STARTING IDs: 28000
== STUDENTS PER IBT: 25
...
-- --- ID Counters ---
DECLARE @StudentID_Start INT = 28000;
DECLARE @JobID_Start INT = 28000;
DECLARE @CertID_Start INT = 28000;
...
INSERT INTO @CertProviders (Name) VALUES (N'Coursera'), (N'Udemy'), (N'Udacity'), (N'Laracasts'), (N'Mahara Tech'), (N'SymfonyCasts'), (N'DataCamp');
...
-- --- Track 15 (PHP Full Stack) Specific Data ---

DECLARE @PHP_Positions TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(100));
INSERT INTO @PHP_Positions (Name) VALUES
(N'PHP Developer'), (N'Laravel Developer'), (N'Full Stack Developer (PHP)'), (N'Backend Developer (PHP)'),
(N'Vue.js Developer'), (N'Junior Web Developer'), (N'Software Engineer - PHP'), (N'Symfony Developer');

DECLARE @PHP_Companies TABLE (Company_ID INT);
INSERT INTO @PHP_Companies (Company_ID) VALUES 
(3),  -- Vodafone Intelligent Solutions (VOIS)
(6),  -- Orange Egypt
(13), -- Etisalat by e&
(14), -- Fawry
(16), -- Robusta
(17), -- Instabug (might use PHP alongside other tech)
(24), -- Raya Holding
(41), -- ArabyAds
(43), -- Paymob
...
DECLARE @PHP_Certs TABLE (ID INT IDENTITY(1,1), Name NVARCHAR(200));
INSERT INTO @PHP_Certs (Name) VALUES
(N'PHP for Beginners (Udemy)'), (N'Laravel 10 From Scratch (Laracasts)'),
(N'Vue - The Complete Guide (Udemy)'), (N'Object Oriented PHP & MVC (Coursera)'),
(N'MySQL for Developers (Udemy)'), (N'Building RESTful APIs with Laravel'),
(N'Symfony 6 Fast Track (SymfonyCasts)'), (N'Mahara Tech PHP Fundamentals');

DECLARE @PHP_Jobs TABLE (ID INT IDENTITY(1,1), Descr NVARCHAR(1000));
INSERT INTO @PHP_Jobs (Descr) VALUES
(N'Build a REST API using Laravel for a mobile app backend.'), (N'Develop a custom authentication system in PHP.'),
(N'Create a new Vue.js component for an existing frontend.'), (N'Fix bugs in a legacy PHP application.'),
(N'Design and implement a MySQL database schema for a project.'), (N'Write unit tests for a Laravel application using PHPUnit.'),
(N'Integrate a third-party API (e.g., weather API) into a Laravel app.'), (N'Build a simple CRUD application using PHP and MySQL without a framework.'),
(N'Optimize slow database queries in a PHP application.'), (N'Set up user roles and permissions in Laravel.'),
(N'Develop a simple job board application with Laravel and Vue.js.'), (N'Refactor procedural PHP code to use OOP principles.'),
(N'Build a small e-commerce product catalog page.'), (N'Implement caching in a Laravel application.'),
(N'Create a command-line script in PHP for data processing.'), (N'Deploy a PHP application to a shared hosting environment.'),
(N'Build a user profile management system.');
...
WHERE ibt.Track_ID = 15; -- PHP Full Stack

-- --- Target Courses (Track 15) ---
...
PRINT 'Starting data generation for Track 15: PHP Full Stack...';
...
BEGIN
    ...
    BEGIN
        ...
        IF RAND() > 0.5 -- Add Facebook randomly
            INSERT INTO Student_Social (Student_ID, Social_Type, Social_Url)
        ...
        BEGIN
            ...
            SELECT TOP 1 @CertName = Name FROM @PHP_Certs ORDER BY NEWID();
            SELECT TOP 1 @CertProvider = Name FROM @CertProviders ORDER BY NEWID();
            SET @CertCost = (ABS(CHECKSUM(NEWID())) % 180) * 10; -- 0 to 1790
        ...
        BEGIN
            ...
            SELECT TOP 1 @JobDesc = Descr FROM @PHP_Jobs ORDER BY NEWID();
            SET @JobEarn = (ABS(CHECKSUM(NEWID())) % 900) + 75; -- 75 to 975
        ...
        BEGIN
            ...
            BEGIN
                ...
                -- Get random company from the PHP list
                SELECT TOP 1 @Company_ID = Company_ID FROM @PHP_Companies ORDER BY NEWID();
                
                -- Get random position
                SELECT TOP 1 @Position = Name FROM @PHP_Positions ORDER BY NEWID();

                -- Get contract type & salary (Full-Time > Part-Time)
                IF RAND() > 0.25
                BEGIN
                    SET @Contract_Type = N'Full-Time';
                    SET @Salary = (ABS(CHECKSUM(NEWID())) % 10000) + 8000; -- 8k to 18k
                END
                ELSE
                BEGIN
...
PRINT 'Data generation complete for Track 15.';
