/*
================================================================================
SQL SCRIPT TO POPULATE STUDENT DATA FOR 'iOS Mobile Application Development' TRACK
================================================================================
This script will:
1.  Define temporary data pools (names, addresses, faculties, etc.).
2.  Set all new primary key counters (Student, Job, Certificate) to start at 12000.
3.  Identify all 'iOS Mobile Application Development' tracks.
...
-- B. Faculties (Weighted for iOS: CS > Eng > Others)
...
-- E. Track-Specific Data (iOS Mobile Application Development, Track_ID = 7)
CREATE TABLE #JobPositions (Position NVARCHAR(100));
INSERT INTO #JobPositions (Position) VALUES
(N'iOS Developer'), (N'Swift Developer'), (N'Mobile Engineer (iOS)'), 
(N'UIKit Developer'), (N'SwiftUI Developer'), (N'iOS Application Engineer'), 
(N'Junior iOS Developer'), (N'Mobile Developer (iOS)');

CREATE TABLE #Certificates (CertName NVARCHAR(200));
INSERT INTO #Certificates (CertName) VALUES
(N'iOS App Development with Swift Specialization'),
(N'UIKit Fundamentals'), (N'SwiftUI Essentials'), 
(N'Core Data for iOS'), (N'Combine Framework Fundamentals'), 
(N'Become an iOS Developer Nanodegree'), (N'iOS & Swift - The Complete iOS App Development Bootcamp');
...
(N'Apple Developer Academy'), (N'Ray Wenderlich'); -- Added Apple and Ray Wenderlich

CREATE TABLE #FreelanceJobs (JobDesc NVARCHAR(1000));
INSERT INTO #FreelanceJobs (JobDesc) VALUES
(N'Build a simple To-Do list app with SwiftUI and Core Data.'),
(N'Convert Figma UI design to native UIKit code.'),
(N'Integrate Firebase Authentication into an existing iOS app.'),
(N'Fix Auto Layout constraints issues on multiple screens.'),
(N'Develop a custom reusable UIView component in Swift.'),
(N'Implement push notifications using APNS and Firebase Cloud Messaging.'),
(N'Create unit tests (XCTest) for an iOS networking layer (URLSession).'),
(N'Build an Apple Watch companion app screen using WatchKit/SwiftUI.'),
(N'Update an old Objective-C codebase to modern Swift syntax.'),
(N'Refactor existing code to use async/await for concurrency.'),
(N'Implement In-App Purchases using StoreKit.'),
(N'Build a MapKit based feature for displaying annotations and routes.'),
(N'Add Core Data persistence to an app currently storing data in memory.'),
(N'Develop a custom iMessage extension for sharing content.'),
(N'Integrate a third-party Swift Package Manager (SPM) library.'),
(N'Implement background fetch and processing tasks.'),
(N'Design and build the UI for a settings screen using UITableView/UICollectionView.');
...
-- F. Relevant Companies for iOS Development
...
WHERE T.Track_Name = N'iOS Mobile Application Development'; -- Track_ID = 7
...
WHERE TC.Track_ID = 7; -- Track_ID for 'iOS Mobile Application Development' is 7

-- ============================================================================
-- STEP 4: DECLARE COUNTERS & CURSOR
-- ============================================================================

-- Global counters starting at 12000
DECLARE @StudentIDCounter INT = 12000;
DECLARE @JobIDCounter INT = 12000;
DECLARE @CertIDCounter INT = 12000;
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @CourseCount INT = (SELECT COUNT(*) FROM #TrackCourses);
         -- Avoid division by zero if no courses found for the track, default duration to 1 day
        ...
        BEGIN
            ...
            INSERT INTO Certificate (
                ...
                CAST(RAND() * 500 + 100 AS DECIMAL(12,2)), -- 100 to 600
                @CertDate
            );
        ...
        BEGIN
            ...
            INSERT INTO Freelance_Job (
                ...
                CAST((RAND() * 800) + 150 AS DECIMAL(12,2)), -- 150 to 950
                @JobDate,
        ...
        BEGIN
            ...
            IF RAND() < @HireChance
            BEGIN
                DECLARE @ContractType NVARCHAR(50) = IIF(RAND() < 0.85, N'Full-Time', N'Part-Time'); -- 85% Full-Time for iOS
                DECLARE @Salary DECIMAL(12,2);
                
                IF @ContractType = N'Full-Time'
                    SET @Salary = CAST((RAND() * 10000) + 12000 AS DECIMAL(12,2)); -- 12k to 22k
                ELSE
                    SET @Salary = CAST((RAND() * 4500) + 6000 AS DECIMAL(12,2)); -- 6k to 10.5k
...
PRINT 'Successfully populated tables with 25 students for each ''iOS Mobile Application Development'' track, starting IDs at 12000.';