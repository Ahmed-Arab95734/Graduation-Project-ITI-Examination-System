/*
================================================================================
SQL SCRIPT TO POPULATE STUDENT DATA FOR 'Python and DevOps Development' TRACK
================================================================================
This script will:
1.  Define temporary data pools (names, addresses, faculties, etc.).
2.  Set all new primary key counters (Student, Job, Certificate) to start at 6000.
3.  Identify all 'Python and DevOps Development' tracks across all intakes and branches.
...
-- B. Faculties (Weighted for Python/DevOps: CS > Eng > Others)
...
-- E. Track-Specific Data (Python and DevOps Development, Track_ID = 4)
CREATE TABLE #JobPositions (Position NVARCHAR(100));
INSERT INTO #JobPositions (Position) VALUES
(N'DevOps Engineer'), (N'Python Developer'), (N'Site Reliability Engineer (SRE)'), 
(N'Cloud Engineer'), (N'Automation Engineer'), (N'CI/CD Specialist'), 
(N'Platform Engineer'), (N'Kubernetes Administrator');
...
(N'Docker Certified Associate (DCA)'),
(N'PCAP: Certified Associate in Python Programming'),
(N'AWS Certified DevOps Engineer - Professional'),
(N'Microsoft Certified: DevOps Engineer Expert (AZ-400)');
...
(N'Linux Foundation'), (N'HashiCorp'), (N'Docker');

CREATE TABLE #FreelanceJobs (JobDesc NVARCHAR(1000));
INSERT INTO #FreelanceJobs (JobDesc) VALUES
(N'Build a CI/CD pipeline using Jenkins for a Django app.'),
(N'Write a Python script to automate file processing from S3.'),
(N'Containerize a Flask application using Docker and Docker Compose.'),
(N'Develop an Ansible playbook to configure web servers.'),
(N'Set up a Kubernetes cluster (EKS/GKE) for a microservice.'),
(N'Write Terraform configuration to deploy a VPC on AWS.'),
(N'Configure Prometheus and Grafana for system monitoring.'),
(N'Automate database backups with a Python script and cron job.'),
(N'Migrate a legacy application to Docker containers.'),
(N'Set up GitHub Actions for automated testing and deployment.'),
(N'Write a script to parse logs and send alerts.'),
(N'Configure an Nginx reverse proxy for a web application.'),
(N'Optimize a slow Python script for data analysis.'),
(N'Build a small API using FastAPI and deploy it to Heroku.'),
(N'Consult on DevOps best practices for a startup.'),
(N'Set up automated unit testing with Pytest for a Python project.');
...
-- F. Relevant Companies for Python and DevOps
...
WHERE T.Track_Name = N'Python and DevOps Development'; -- Track_ID = 4
...
WHERE TC.Track_ID = 4; -- Track_ID for 'Python and DevOps Development' is 4

-- ============================================================================
-- STEP 4: DECLARE COUNTERS & CURSOR
-- ============================================================================

-- Global counters starting at 6000
DECLARE @StudentIDCounter INT = 6000;
DECLARE @JobIDCounter INT = 6000;
DECLARE @CertIDCounter INT = 6000;
...
BEGIN
    ...
    BEGIN
        ...
        DECLARE @StudentAddress NVARCHAR(255) = N'101 DevOps Ave, ' + @StudentGov;
        ...
        BEGIN
            ...
            INSERT INTO Freelance_Job (
                ...
                CAST((RAND() * 900) + 100 AS DECIMAL(12,2)), -- 100 to 1000
                @JobDate,
        ...
        BEGIN
            ...
            IF RAND() < @HireChance
            BEGIN
                DECLARE @ContractType NVARCHAR(50) = IIF(RAND() < 0.85, N'Full-Time', N'Part-Time'); -- 85% Full-Time
                DECLARE @Salary DECIMAL(12,2);
                
                IF @ContractType = N'Full-Time'
                    SET @Salary = CAST((RAND() * 11000) + 12000 AS DECIMAL(12,2)); -- 12k to 23k
                ELSE
                    SET @Salary = CAST((RAND() * 5000) + 6000 AS DECIMAL(12,2)); -- 6k to 11k
...
PRINT 'Successfully populated tables with 25 students for each ''Python and DevOps Development'' track, starting IDs at 6000.';