/********************************************************************************
* RUN SCRIPT
* Description: Populates Student and related tables for Track 'Data Visualization' (ID 23)
* Start IDs: 44000 for Student, Job, and Certificate
* Students per IBT: 25
*
* Rules Enforced:
* 1.  Only for Track_ID 23.
* 2.  25 Students per Intake_Branch_Track (IBT).
* 3.  Unique IDs starting from 44000.
...
* 9.  Faculties weighted towards 'Computers Sciences' and 'Engineering'.
...
* 16. Positions, Companies, Certs, Freelance jobs are all relevant to 'Data Visualization'.
* 17. Certs/Freelance jobs dated *during* the intake period.
* 18. Student_Course table populated sequentially for all 12 'Data Visualization' courses.
...
(N'Faculty of Science'), (N'Faculty of Commerce'), (N'Faculty of Business Administration'),
(N'Faculty of Information Systems'), (N'Faculty of Education'), (N'Faculty of Economics and Political Science'),
(N'Faculty of Agriculture'), (N'Faculty of Fine Arts'), (N'Faculty of Applied Arts'), (N'Faculty of Arts');

-- -- -- -- -- -- -- -- -- -- -- --
-- Track-Specific Data: Data Visualization (Track 23)
-- -- -- -- -- -- -- -- -- -- -- --
...
FROM Track_Course WHERE Track_ID = 23;

DECLARE @Companies_DV TABLE (ID INT);
INSERT INTO @Companies_DV VALUES
(1), (3), (4), (6), (8), (10), (13), (14), (15), (16), (21), (22), (23), (24), (27), (28), (29), (30), (41), (42), (43), (46), (53); -- Microsoft, VOIS, Accenture, Orange, CIB, IBM, Etisalat, Fawry, NAGWA, Robusta, NBE, Banque Misr, WE, Raya, PwC, Deloitte, EY, KPMG, ArabyAds, e-finance, Paymob, Talabat, EFG Hermes

DECLARE @Positions_DV TABLE (Name NVARCHAR(100));
INSERT INTO @Positions_DV VALUES
(N'Data Visualization Specialist'), (N'BI Developer'), (N'Data Analyst'), (N'Tableau Developer'),
(N'Power BI Developer'), (N'Reporting Analyst'), (N'Dashboard Designer'), (N'Business Intelligence Analyst');

DECLARE @CertProviders TABLE (Name NVARCHAR(200));
INSERT INTO @CertProviders VALUES (N'Udemy'), (N'Coursera'), (N'Udacity'), (N'Mahara Tech'), (N'DataCamp'), (N'Microsoft');

DECLARE @CertNames_DV TABLE (Name NVARCHAR(200));
INSERT INTO @CertNames_DV VALUES
(N'Tableau Desktop Specialist Certification Prep'), (N'Microsoft Power BI Data Analyst (PL-300) Prep'), (N'Data Visualization with Python (Matplotlib/Seaborn)'),
(N'SQL for Data Science'), (N'Data Storytelling for Business'), (N'Advanced D3.js');
...
DECLARE @FreelanceJobs_DV TABLE (Name NVARCHAR(1000));
INSERT INTO @FreelanceJobs_DV VALUES
(N'Built an interactive sales dashboard in Tableau.'), (N'Created a set of KPIs reports in Power BI.'),
(N'Designed 5 infographics for a marketing campaign.'), (N'Cleaned and prepared sales data for visualization.'),
(N'Developed a custom D3.js chart for a website.'), (N'Created a presentation summarizing key data insights.'),
(N'Automated a weekly reporting process using SQL and Excel.'), (N'Built a financial performance dashboard for a small business.'),
(N'Analyzed customer survey data and visualized results.'), (N'Created a Tableau Story to present market trends.'),
(N'Designed a Power BI report with drill-through capabilities.'), (N'Wrote SQL queries to extract data for reporting.'),
(N'Visualized geographical data using Tableau Maps.'), (N'Created animated data visualizations for a presentation.'),
(N'Developed a dashboard monitoring website traffic in Google Analytics.');


-- =============================================================================
-- 2. DECLARE CURSOR AND LOOP VARIABLES
-- =============================================================================
DECLARE @CurrentID INT = 44000;
DECLARE @CurrentJobID INT = 44000;
DECLARE @CurrentCertID INT = 44000;
...
DECLARE IBT_Cursor CURSOR FOR
    ...
    WHERE
        IBT.Track_ID = 23; -- <<<<<<< ONLY 'Data Visualization'
...
BEGIN
    ...
    BEGIN
        ...
        -- -- -- -- -- -- -- -- -- -- -- --
        -- 4.3. Generate Weighted Faculty
        -- -- -- -- -- -- -- -- -- -- -- --
        SET @FacultyRand = RAND();
        IF @FacultyRand < 0.35 -- 35% chance
            SET @Faculty = N'Faculty of Computers Sciences';
        ELSE IF @FacultyRand < 0.60 -- 25% chance (total 60%)
        ...
        (@StudentID, N'GitHub', N'https://github.com/' + LOWER(REPLACE(@Fname, ' ', '')) + LOWER(REPLACE(@Lname, ' ', '')) + CAST(@StudentID % 1000 AS VARCHAR(4))),
        ...
        BEGIN
            ...
            SET @CertName = (SELECT TOP 1 Name FROM @CertNames_DV ORDER BY NEWID());
            SET @CertProvider = (SELECT TOP 1 Name FROM @CertProviders ORDER BY NEWID());
            SET @CertCost = CAST(RAND() * 500 AS DECIMAL(12,2));
        ...
        BEGIN
            ...
            SET @JobDesc = (SELECT TOP 1 Name FROM @FreelanceJobs_DV ORDER BY NEWID());
        ...
        BEGIN
            SET @CompanyID = (SELECT TOP 1 ID FROM @Companies_DV ORDER BY NEWID());
            SET @Position = (SELECT TOP 1 Name FROM @Positions_DV ORDER BY NEWID());
            SET @Contract = CASE WHEN RAND() > 0.7 THEN N'Full-Time' ELSE N'Part-Time' END;
            
            -- Full-Time (8.5k-17k) > Part-Time (4.5k-8.5k)
            SET @Salary = CASE @Contract
                WHEN N'Full-Time' THEN CAST(RAND() * 8500 + 8500 AS DECIMAL(12,2))
                ELSE CAST(RAND() * 4000 + 4500 AS DECIMAL(12,2))
            END;
...
PRINT 'Successfully populated database for Track 23 (Data Visualization).';
PRINT 'Total Students Inserted: ' + CAST((@CurrentID - 44000) AS VARCHAR(10));
