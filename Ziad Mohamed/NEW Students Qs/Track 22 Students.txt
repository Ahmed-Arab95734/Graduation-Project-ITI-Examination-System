/********************************************************************************
* RUN SCRIPT
* Description: Populates Student and related tables for Track 'Cybersecurity Associate' (ID 22)
* Start IDs: 42000 for Student, Job, and Certificate
* Students per IBT: 25
*
* Rules Enforced:
* 1.  Only for Track_ID 22.
* 2.  25 Students per Intake_Branch_Track (IBT).
* 3.  Unique IDs starting from 42000.
...
* 16. Positions, Companies, Certs, Freelance jobs are all relevant to 'Cybersecurity Associate'.
* 17. Certs/Freelance jobs dated *during* the intake period.
* 18. Student_Course table populated sequentially for all 12 'Cybersecurity Associate' courses.
...
-- -- -- -- -- -- -- -- -- -- -- --
-- Track-Specific Data: Cybersecurity Associate (Track 22)
-- -- -- -- -- -- -- -- -- -- -- --
...
FROM Track_Course WHERE Track_ID = 22;

DECLARE @Companies_CS TABLE (ID INT);
INSERT INTO @Companies_CS VALUES
(1), (2), (3), (4), (6), (8), (10), (12), (13), (14), (21), (22), (23), (24), (42), (47), (48), (49); -- Microsoft, AWS, VOIS, Accenture, Orange, CIB, IBM, Dell, Etisalat, Fawry, NBE, Banque Misr, WE, Raya, e-finance, Kyndryl, Atos, Cisco

DECLARE @Positions_CS TABLE (Name NVARCHAR(100));
INSERT INTO @Positions_CS VALUES
(N'Cybersecurity Analyst'), (N'SOC Analyst L1'), (N'Penetration Tester (Junior)'), (N'IT Security Specialist'),
(N'Network Security Engineer'), (N'Incident Responder'), (N'GRC Analyst'), (N'Security Consultant');
...
DECLARE @CertNames_CS TABLE (Name NVARCHAR(200));
INSERT INTO @CertNames_CS VALUES
(N'CompTIA Security+ Prep'), (N'Certified Ethical Hacker (CEH) Prep'), (N'Cisco CCNA Security'),
(N'Palo Alto Networks (PCNSA) Prep'), (N'Intro to SIEM (Splunk)'), (N'Network Forensics Basics');
...
DECLARE @FreelanceJobs_CS TABLE (Name NVARCHAR(1000));
INSERT INTO @FreelanceJobs_CS VALUES
(N'Performed vulnerability scan on a web app.'), (N'Configured firewall rules for a small office.'),
(N'Wrote a security policy document.'), (N'Analyzed phishing emails for a client.'),
(N'Hardened a Linux server.'), (N'Set up a VPN for a remote team.'),
(N'Performed a network security audit.'), (N'Scanned a network for open ports and services.'),
(N'Assisted in a malware removal process.'), (N'Wrote a Python script for log analysis.'),
(N'Configured fail2ban on a web server.'), (N'Performed a simple penetration test on a test website.'),
(N'Set up multi-factor authentication for a client''s service.'), (N'Researched a new security vulnerability and wrote a summary.'),
(N'Configured an intrusion detection system (IDS).');


-- =============================================================================
-- 2. DECLARE CURSOR AND LOOP VARIABLES
-- =============================================================================
DECLARE @CurrentID INT = 42000;
DECLARE @CurrentJobID INT = 42000;
DECLARE @CurrentCertID INT = 42000;
...
DECLARE IBT_Cursor CURSOR FOR
    ...
    WHERE
        IBT.Track_ID = 22; -- <<<<<<< ONLY 'Cybersecurity Associate'
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET @CertName = (SELECT TOP 1 Name FROM @CertNames_CS ORDER BY NEWID());
        ...
        BEGIN
            ...
            SET @JobDesc = (SELECT TOP 1 Name FROM @FreelanceJobs_CS ORDER BY NEWID());
        ...
        BEGIN
            SET @CompanyID = (SELECT TOP 1 ID FROM @Companies_CS ORDER BY NEWID());
            SET @Position = (SELECT TOP 1 Name FROM @Positions_CS ORDER BY NEWID());
...
PRINT 'Successfully populated database for Track 22 (Cybersecurity Associate).';
PRINT 'Total Students Inserted: ' + CAST((@CurrentID - 42000) AS VARCHAR(10));
