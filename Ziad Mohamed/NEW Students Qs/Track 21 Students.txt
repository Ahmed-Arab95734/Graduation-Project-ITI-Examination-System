/********************************************************************************
* RUN SCRIPT
* Description: Populates Student and related tables for Track 'Systems Administration' (ID 21)
* Start IDs: 40000 for Student, Job, and Certificate
* Students per IBT: 25
*
* Rules Enforced:
* 1.  Only for Track_ID 21.
* 2.  25 Students per Intake_Branch_Track (IBT).
* 3.  Unique IDs starting from 40000.
...
* 16. Positions, Companies, Certs, Freelance jobs are all relevant to 'Systems Administration'.
* 17. Certs/Freelance jobs dated *during* the intake period.
* 18. Student_Course table populated sequentially for all 12 'Systems Administration' courses.
...
-- -- -- -- -- -- -- -- -- -- -- --
-- Track-Specific Data: Systems Administration (Track 21)
-- -- -- -- -- -- -- -- -- -- -- --
...
FROM Track_Course WHERE Track_ID = 21;

DECLARE @Companies_SA TABLE (ID INT);
INSERT INTO @Companies_SA VALUES
(1), (2), (3), (6), (7), (8), (10), (12), (13), (14), (23), (24), (42), (47), (48), (49); -- Microsoft, AWS, VOIS, Orange, Valeo, CIB, IBM, Dell, Etisalat, Fawry, WE, Raya, e-finance, Kyndryl, Atos, Cisco

DECLARE @Positions_SA TABLE (Name NVARCHAR(100));
INSERT INTO @Positions_SA VALUES
(N'Systems Administrator'), (N'Network Administrator'), (N'IT Support Specialist'), (N'Linux Administrator'),
(N'Windows Server Administrator'), (N'Cloud Support Engineer'), (N'Infrastructure Engineer'), (N'Junior DevOps Engineer');
...
DECLARE @CertNames_SA TABLE (Name NVARCHAR(200));
INSERT INTO @CertNames_SA VALUES
(N'Red Hat Certified System Administrator (RHCSA) Prep'), (N'Microsoft Certified: Azure Administrator Associate'), (N'CompTIA Linux+'),
(N'Windows Server Administration Fundamentals'), (N'VMware VCP Prep'), (N'AWS Certified SysOps Administrator');
...
DECLARE @FreelanceJobs_SA TABLE (Name NVARCHAR(1000));
INSERT INTO @FreelanceJobs_SA VALUES
(N'Set up Active Directory for a small business.'), (N'Configured a Linux web server (LAMP stack).'),
(N'Wrote a PowerShell script to automate user backups.'), (N'Troubleshot network connectivity issues for a remote office.'),
(N'Hardened a Windows Server 2022 instance.'), (N'Migrated a small website to a new host.'),
(N'Managed user permissions and group policies.'), (N'Set up a pfSense firewall.'),
(N'Configured a VPN for remote access.'), (N'Provided on-call IT support.'),
(N'Monitored server performance and resolved alerts.'), (N'Automated a task using Bash scripting.'),
(N'Configured network switches and routers.'), (N'Set up a local backup solution.'),
(N'Assisted with a cloud migration to AWS EC2.');


-- =============================================================================
-- 2. DECLARE CURSOR AND LOOP VARIABLES
-- =============================================================================
DECLARE @CurrentID INT = 40000;
DECLARE @CurrentJobID INT = 40000;
DECLARE @CurrentCertID INT = 40000;
...
DECLARE IBT_Cursor CURSOR FOR
    ...
    WHERE
        IBT.Track_ID = 21; -- <<<<<<< ONLY 'Systems Administration'
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET @CertName = (SELECT TOP 1 Name FROM @CertNames_SA ORDER BY NEWID());
        ...
        BEGIN
            ...
            SET @JobDesc = (SELECT TOP 1 Name FROM @FreelanceJobs_SA ORDER BY NEWID());
        ...
        BEGIN
            SET @CompanyID = (SELECT TOP 1 ID FROM @Companies_SA ORDER BY NEWID());
            SET @Position = (SELECT TOP 1 Name FROM @Positions_SA ORDER BY NEWID());
...
PRINT 'Successfully populated database for Track 21 (Systems Administration).';
PRINT 'Total Students Inserted: ' + CAST((@CurrentID - 40000) AS VARCHAR(10));
