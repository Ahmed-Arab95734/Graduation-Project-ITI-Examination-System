/********************************************************************************
* RUN SCRIPT
* Description: Populates Student and related tables for Track 'UI/UX Design' (ID 20)
* Start IDs: 38000 for Student, Job, and Certificate
* Students per IBT: 25
*
* Rules Enforced:
* 1.  Only for Track_ID 20.
* 2.  25 Students per Intake_Branch_Track (IBT).
* 3.  Unique IDs starting from 38000.
...
* 16. Positions, Companies, Certs, Freelance jobs are all relevant to 'UI/UX Design'.
* 17. Certs/Freelance jobs dated *during* the intake period.
* 18. Student_Course table populated sequentially for all 12 'UI/UX Design' courses.
...
-- -- -- -- -- -- -- -- -- -- -- --
-- Track-Specific Data: UI/UX Design (Track 20)
-- -- -- -- -- -- -- -- -- -- -- --
...
FROM Track_Course WHERE Track_ID = 20;

DECLARE @Companies_UIUX TABLE (ID INT);
INSERT INTO @Companies_UIUX VALUES
(3), (6), (8), (13), (14), (15), (16), (17), (23), (24), (41), (43), (46), (50); -- VOIS, Orange, CIB, Etisalat, Fawry, NAGWA, Robusta, Instabug, WE, Raya, ArabyAds, Paymob, Talabat, Jumia

DECLARE @Positions_UIUX TABLE (Name NVARCHAR(100));
INSERT INTO @Positions_UIUX VALUES
(N'UI/UX Designer'), (N'Product Designer'), (N'UX Researcher'), (N'Interaction Designer'),
(N'UI Designer'), (N'Visual Designer'), (N'Usability Analyst'), (N'UX Writer');
...
DECLARE @CertNames_UIUX TABLE (Name NVARCHAR(200));
INSERT INTO @CertNames_UIUX VALUES
(N'Figma UI/UX Design Essentials'), (N'User Research and Personas'), (N'Prototyping and Usability Testing'),
(N'Google UX Design Professional Certificate'), (N'Interaction Design Specialization'), (N'Accessibility: How to Design for All');
...
DECLARE @FreelanceJobs_UIUX TABLE (Name NVARCHAR(1000));
INSERT INTO @FreelanceJobs_UIUX VALUES
(N'Designed a 5-screen mobile app prototype in Figma.'),
(N'Conducted 5 user interviews and created 2 personas for a startup.'),
(N'Created a high-fidelity web landing page design for a tech company.'),
(N'Performed a heuristic evaluation of an existing e-commerce website.'),
(N'Designed a style guide and component library for a SaaS product.'),
(N'Wireframed a new user onboarding flow for a mobile app.'),
(N'Redesigned the checkout process for an online store.'),
(N'Created user flow diagrams for a complex dashboard interface.'),
(N'Designed a set of 50 custom icons for an application.'),
(N'Ran a usability test session with 3 users and compiled a report.'),
(N'Designed a responsive email newsletter template.'),
(N'Created a low-fidelity prototype for a new feature idea.'),
(N'Developed an information architecture plan for a new website.'),
(N'Designed a component-based UI kit in Figma.'),
(N'Crafted UX microcopy for error messages and success states.');


-- =============================================================================
-- 2. DECLARE CURSOR AND LOOP VARIABLES
-- =============================================================================
DECLARE @CurrentID INT = 38000;
DECLARE @CurrentJobID INT = 38000;
DECLARE @CurrentCertID INT = 38000;
...
DECLARE IBT_Cursor CURSOR FOR
    ...
    WHERE
        IBT.Track_ID = 20; -- <<<<<<< ONLY 'UI/UX Design'
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET @CertName = (SELECT TOP 1 Name FROM @CertNames_UIUX ORDER BY NEWID());
        ...
        BEGIN
            ...
            SET @JobDesc = (SELECT TOP 1 Name FROM @FreelanceJobs_UIUX ORDER BY NEWID());
        ...
        BEGIN
            SET @CompanyID = (SELECT TOP 1 ID FROM @Companies_UIUX ORDER BY NEWID());
            SET @Position = (SELECT TOP 1 Name FROM @Positions_UIUX ORDER BY NEWID());
...
PRINT 'Successfully populated database for Track 20 (UI/UX Design).';
PRINT 'Total Students Inserted: ' + CAST((@CurrentID - 38000) AS VARCHAR(10));
