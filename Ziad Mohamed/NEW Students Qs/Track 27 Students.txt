/********************************************************************************
* RUN SCRIPT
* Description: Populates Student and related tables for Track 'Social Media Marketing' (ID 27)
* Start IDs: 52000 for Student, Job, and Certificate
* Students per IBT: 25
*
* Rules Enforced:
* 1.  Only for Track_ID 27.
* 2.  25 Students per Intake_Branch_Track (IBT).
* 3.  Unique IDs starting from 52000.
...
* 8.  Social links include student name (LinkedIn, Facebook, Instagram relevant).
* 9.  Faculties weighted towards Business/Commerce/Arts/IS/Econ, less on pure Tech.
...
* 15. Salaries: Full-Time > Part-Time (adjusted for SMM roles).
* 16. Positions, Companies, Certs, Freelance jobs relevant to 'Social Media Marketing'.
...
-- -- -- -- -- -- -- -- -- -- -- --
-- Faculties (Weighted) - Adjusted for Social Media Marketing
-- -- -- -- -- -- -- -- -- -- -- --
...
DECLARE @ArtsFaculties TABLE (Name NVARCHAR(100));
INSERT INTO @ArtsFaculties VALUES (N'Faculty of Arts'), (N'Faculty of Education'), (N'Faculty of Applied Arts'), (N'Faculty of Fine Arts');
...
INSERT INTO @OtherFaculties VALUES (N'Faculty of Science'), (N'Faculty of Agriculture');


-- -- -- -- -- -- -- -- -- -- -- --
-- Track-Specific Data: Social Media Marketing (Track 27)
-- -- -- -- -- -- -- -- -- -- -- --
...
FROM Track_Course WHERE Track_ID = 27;

DECLARE @Companies_SMM TABLE (ID INT);
INSERT INTO @Companies_SMM VALUES
(3), (6), (8), (13), (14), (16), (23), (24), (36), (37), (38), (40), (41), (43), (46), (50), (52); -- VOIS, Orange, CIB, Etisalat, Fawry, Robusta, WE, Raya, P&G, Unilever, Nestle, Majid Al Futtaim, ArabyAds, Paymob, Talabat, Jumia, L'Oreal

DECLARE @Positions_SMM TABLE (Name NVARCHAR(100));
INSERT INTO @Positions_SMM VALUES
(N'Social Media Specialist'), (N'Content Creator'), (N'Community Manager'), (N'Digital Marketing Specialist'),
(N'SEO Specialist'), (N'PPC Specialist'), (N'Marketing Coordinator'), (N'Social Media Analyst');

DECLARE @CertProviders TABLE (Name NVARCHAR(200));
INSERT INTO @CertProviders VALUES (N'Udemy'), (N'Coursera'), (N'Udacity'), (N'Mahara Tech'), (N'DataCamp'), (N'Microsoft'), (N'Google'), (N'Meta'), (N'Hootsuite'); -- Added Platform specific

DECLARE @CertNames_SMM TABLE (Name NVARCHAR(200));
INSERT INTO @CertNames_SMM VALUES
(N'Meta Certified Digital Marketing Associate'), (N'Google Ads Display Certification'), (N'Hootsuite Platform Certification'),
(N'Content Marketing Certified (HubSpot)'), (N'SEO Fundamentals Exam (SEMRush)'), (N'Email Marketing Certification');
...
DECLARE @FreelanceJobs_SMM TABLE (Name NVARCHAR(1000));
INSERT INTO @FreelanceJobs_SMM VALUES
(N'Managed Facebook and Instagram pages for a local cafe.'), (N'Ran a lead generation campaign on LinkedIn Ads.'),
(N'Wrote 5 blog posts on digital marketing trends.'), (N'Created a monthly social media content calendar.'),
(N'Analyzed social media engagement metrics and created a report.'), (N'Performed SEO keyword research for a new website.'),
(N'Set up and managed a Google Ads campaign for an e-commerce store.'), (N'Designed social media graphics using Canva.'),
(N'Responded to comments and messages on social media channels.'), (N'Developed a short video script for a TikTok ad.'),
(N'Created an email marketing sequence in Mailchimp.'), (N'Analyzed competitor social media strategies.'),
(N'Set up Google Analytics tracking for a website.'), (N'Wrote ad copy for Facebook campaigns.'),
(N'Managed influencer collaborations for a brand.');


-- =============================================================================
-- 2. DECLARE CURSOR AND LOOP VARIABLES
-- =============================================================================
DECLARE @CurrentID INT = 52000;
DECLARE @CurrentJobID INT = 52000;
DECLARE @CurrentCertID INT = 52000;
DECLARE @StudentCounter INT;

-- -- -- -- -- -- -- -- -- -- -- -- IBT-level Variables -- -- -- -- -- -- -- -- -- -- -- --
DECLARE @IBT_ID INT, @Intake_ID INT, @Branch_ID INT, @Track_ID INT;
DECLARE @Intake_Start_Date DATE, @Intake_End_Date DATE;
DECLARE @Branch_Location NVARCHAR(100);

-- -- -- -- -- -- -- -- -- -- -- -- Student-level Variables -- -- -- -- -- -- -- -- -- -- -- --
DECLARE @StudentID INT, @Fname NVARCHAR(50), @Lname NVARCHAR(50), @Gender NVARCHAR(10);
DECLARE @MaritalStatus NVARCHAR(50), @Birthdate DATE, @Governorate NVARCHAR(50);
DECLARE @Street NVARCHAR(100), @Address NVARCHAR(255), @Email NVARCHAR(100), @Phone NVARCHAR(20);
DECLARE @Faculty NVARCHAR(100), @FacultyGrade NVARCHAR(50), @FacultyRand FLOAT;
DECLARE @Status NVARCHAR(50), @Hired BIT, @HiringProb FLOAT;
...
DECLARE @TotalCourses INT = (SELECT COUNT(*) FROM @TrackCourses), @TotalDuration INT, @CourseDuration INT;
DECLARE @Course_i INT, @CurrentCourse_ID INT, @CurrentCourseStart DATE, @CurrentCourseEnd DATE;

-- -- -- -- -- -- -- -- -- -- -- -- Main Cursor -- -- -- -- -- -- -- -- -- -- -- --
DECLARE IBT_Cursor CURSOR FOR
    SELECT IBT.Intake_Branch_Track_ID, IBT.Intake_ID, IBT.Branch_ID, IBT.Track_ID,
           I.Intake_Start_Date, I.Intake_End_Date, B.Branch_Location
    FROM Intake_Branch_Track AS IBT
    JOIN Intake AS I ON IBT.Intake_ID = I.Intake_ID
    JOIN Branch AS B ON IBT.Branch_ID = B.Branch_ID
    WHERE IBT.Track_ID = 27; -- <<<<<<< ONLY 'Social Media Marketing'
...
BEGIN
    ...
    SET @CourseDuration = @TotalDuration / @TotalCourses;

    -- 3.1. Determine Branch-specific Hiring Probability
    SET @HiringProb = CASE @Branch_ID WHEN 1 THEN 0.40 WHEN 12 THEN 0.30 WHEN 11 THEN 0.25 WHEN 2 THEN 0.20 ELSE 0.10 END;
    ...
    BEGIN
        ...
        IF @Gender = N'Male' SET @Fname = (SELECT TOP 1 Name FROM @MaleNames ORDER BY NEWID());
        ELSE SET @Fname = (SELECT TOP 1 Name FROM @FemaleNames ORDER BY NEWID());
        ...
        SET @Governorate = (SELECT TOP 1 Gov_Name FROM @Governorates WHERE Branch_Locations LIKE '%,' + @Branch_Location + ',%' ORDER BY NEWID());
        ...
        -- 4.3. Generate Weighted Faculty (Adjusted for SMM)
        SET @FacultyRand = RAND();
        IF @FacultyRand < 0.35 -- 35% chance Business/Commerce/IS/Econ
            SET @Faculty = (SELECT TOP 1 Name FROM @BusinessFaculties ORDER BY NEWID());
        ELSE IF @FacultyRand < 0.55 -- 20% chance Arts/Edu (total 55%)
            SET @Faculty = (SELECT TOP 1 Name FROM @ArtsFaculties ORDER BY NEWID());
        ELSE IF @FacultyRand < 0.70 -- 15% chance Tech (CS/Eng) (total 70%)
             SET @Faculty = (SELECT TOP 1 Name FROM @TechFaculties ORDER BY NEWID());
        ELSE -- 30% chance Others (Sci/Agri)
        ...
        BEGIN
            IF RAND() < 0.05 SET @Status = N'Failed to Graduate';
            ELSE BEGIN SET @Status = N'Graduated'; IF RAND() < @HiringProb SET @Hired = 1; END;
        END
        ELSE SET @Status = N'In Progress';
        ...
        VALUES (@StudentID, @Email, @Address, @Gender, @MaritalStatus, @Fname, @Lname, @Birthdate, @Faculty, @FacultyGrade, @Status, @IBT_ID);

        -- 4.7. INSERT Student_Phone
        INSERT INTO Student_Phone (Student_ID, Phone) VALUES (@StudentID, @Phone);

        -- 4.8. INSERT Student_Social (Added Instagram, Removed GitHub)
        ...
        (@StudentID, N'Instagram', N'https://instagram.com/' + LOWER(REPLACE(@Fname, ' ', '')) + LOWER(REPLACE(@Lname, ' ', '')) + CAST(@StudentID % 1000 AS VARCHAR(4))),
        ...
        SET @Course_i = 1; SET @CurrentCourseStart = @Intake_Start_Date;
        ...
        BEGIN
            ...
            IF @Course_i = @TotalCourses SET @CurrentCourseEnd = @Intake_End_Date;
            INSERT INTO Student_Course (Student_ID, Course_ID, Course_StartDate, Course_EndDate) VALUES (@StudentID, @CurrentCourse_ID, @CurrentCourseStart, @CurrentCourseEnd);
            SET @CurrentCourseStart = DATEADD(DAY, 1, @CurrentCourseEnd); SET @Course_i = @Course_i + 1;
        END;

        -- 4.10. INSERT Certificates (Random 0-3)
        SET @NumCerts = CAST(RAND() * 4 AS INT); SET @Cert_i = 0;
        WHILE @Cert_i < @NumCerts
        BEGIN
            SET @CertID = @CurrentCertID;
            SET @CertName = (SELECT TOP 1 Name FROM @CertNames_SMM ORDER BY NEWID());
            SET @CertProvider = (SELECT TOP 1 Name FROM @CertProviders ORDER BY NEWID());
            SET @CertCost = CAST(RAND() * 300 AS DECIMAL(12,2)); -- SMM certs often cheaper or free
            SET @CertDate = DATEADD(DAY, RAND() * @TotalDuration, @Intake_Start_Date);
            INSERT INTO Certificate (Certificate_ID, Student_ID, Certificate_Name, Certificate_Provider, Certificate_Cost, Certificate_Date) VALUES (@CertID, @StudentID, @CertName, @CertProvider, @CertCost, @CertDate);
            SET @CurrentCertID = @CurrentCertID + 1; SET @Cert_i = @Cert_i + 1;
        END;

        -- 4.11. INSERT Freelance_Job (Random 0-4)
        SET @NumJobs = CAST(RAND() * 5 AS INT); SET @Job_i = 0;
        WHILE @Job_i < @NumJobs
        BEGIN
            SET @JobID = @CurrentJobID;
            SET @JobSite = (SELECT TOP 1 Name FROM @FreelanceSites ORDER BY NEWID());
            SET @JobDesc = (SELECT TOP 1 Name FROM @FreelanceJobs_SMM ORDER BY NEWID());
            SET @JobEarn = CAST(RAND() * 800 + 40 AS DECIMAL(12,2)); -- Slightly lower avg earn for SMM gigs typically
            SET @JobDate = DATEADD(DAY, RAND() * @TotalDuration, @Intake_Start_Date);
            INSERT INTO Freelance_Job (Job_ID, Student_ID, Job_Earn, Job_Date, Job_Site, Description) VALUES (@JobID, @StudentID, @JobEarn, @JobDate, @JobSite, @JobDesc);
            SET @CurrentJobID = @CurrentJobID + 1; SET @Job_i = @Job_i + 1;
        END;

        -- 4.12. INSERT Student_Company (IF Graduated & Hired)
        IF @Status = N'Graduated' AND @Hired = 1
        BEGIN
            SET @CompanyID = (SELECT TOP 1 ID FROM @Companies_SMM ORDER BY NEWID());
            SET @Position = (SELECT TOP 1 Name FROM @Positions_SMM ORDER BY NEWID());
            SET @Contract = CASE WHEN RAND() > 0.7 THEN N'Full-Time' ELSE N'Part-Time' END;
            -- Full-Time (7k-14k) > Part-Time (3.5k-7k) - Adjusted for SMM
            SET @Salary = CASE @Contract WHEN N'Full-Time' THEN CAST(RAND() * 7000 + 7000 AS DECIMAL(12,2)) ELSE CAST(RAND() * 3500 + 3500 AS DECIMAL(12,2)) END;
            SET @HireDate = DATEADD(DAY, RAND() * DATEDIFF(DAY, @Intake_End_Date, GETDATE()), @Intake_End_Date);
            INSERT INTO Student_Company (Student_ID, Company_ID, Salary, Position, Contract_Type, Hire_Date, Leave_Date) VALUES (@StudentID, @CompanyID, @Salary, @Position, @Contract, @HireDate, NULL);
        END;
...
PRINT 'Successfully populated database for Track 27 (Social Media Marketing).';
PRINT 'Total Students Inserted: ' + CAST((@CurrentID - 52000) AS VARCHAR(10));
